...
**この計画ファイルは M76h (sub-08、実装完了後) にコピーした記録**。design-draft.md に
書かれていた「実装開始時にコピーする」「worktree + ブランチで進める」という手順は、
実際には `/harness` (planner/coder/reviewer の 3 役) で master 上のセッション単位に
進める運用に差し替わった (`plans\m76-deepmodal\harness.md` が実際の進捗台帳)。
以下は sub-01〜sub-09 の**出発点となった設計案**であり、実装中に確定した仕様は
`plans\m76-deepmodal\spec.md` (差分は spec §8 の変更履歴)、実際の進捗は
`git log` (`M76a`〜`M76h`) が正本。本ファイル末尾に完了時点の申し送りを追記した。
...
## M76 完了時点の申し送り (M76h、2026-09-16)

**実施順序・体制の差分 (design-draft.md からの変更)**:
- worktree + 専用ブランチではなく、`/harness` (planner/coder/reviewer) で master 上に
  1 サブ = 1 コミットとして進めた (M75 が既に master へ入っていたため、当初の「M75 が
  master で継続中なので worktree」という前提が外れた)。
- サブ分割は sub-01〜sub-09 (M76a〜M76h + M76e2) で、design-draft.md の想定に
  「sub-09 (M76e2): CPU 推論の SIMD/マルチスレッド最適化」がユーザー判断で追加された
  (sub-05 の推論時間が実測 4.75–5.34 s/メッシュだったため)。
- ADR の番号は design-draft.md が想定していた **ADR-021 ではなく ADR-020**
  (実装時点で ADR-020 が空いていた — M75 系の ADR がまだ確定していなかったため)。

**設計判断で確定と異なった点 (詳細は spec.md §2・§8、各 sub-NN.md のフィードバック履歴)**:
- `poissonRatio` は当初 planner が「PhysMat に足さない」と裁定したが、ユーザー判断で
  「保持する (ランタイムは読まない)」へ変更。
- 固有値解法は「cap 9000 + 不完全 LU 前処理 LOBPCG + 残差による採否」に確定
  (design-draft.md には無かった詳細)。
- Mel-band coverage という 3 つ目の品質指標をユーザー追加指示で新設。
- `L_ref` (design-draft.md では 0.3 m の想定) は M76h で **0.6 m へ改訂**
  (stage0 全体で mode_count 中央値 14→39.5、coverage_ratio 0.336→0.520 の改善を実測)。
- overfit の門は「mask acc > 99% かつ amp MSE < 1e-3」の想定だったが、
  「mask acc > 99% かつ amp の説明率 R² ≥ 0.90」へ確定 (絶対 MSE の閾値は表現力の
  下限を下回っていて到達不能だったため)。

**M76h (このファイルへの記録時点) のデータ段階**: stage1 (小規模自前、`assets\models`
から選んだ 87 サブメッシュ、`tools\deepmodal\small.txt`) で `dataset → train → export →
--modal-bake → --modal-demo` が端から端まで通り、`assets\deepmodal\deepmodal.dmnet`
(3,368,464 B、paramCount=1,682,448、**pooled R²=0.6277 / mask_acc=99.82%** — round 2 の
must #1 で本学習の設定 (epochs/lr 半減/下限) を直した後の値) をコミットした。
ModelNet10/40 は `tools\deepmodal\README.md` の手順でユーザーが別途実行する
(実行禁止の門は overfit gate 通過済みで解除されているが、実走は本 M76 の範囲外)。
★round 1 は本学習の pooled R² を測らずに amp_mse (サンプル毎正規化) だけで報告しており、
実際には pooled R²=-0.2536 (定数モデルより悪い) という underfit を見落としていた
(sub-08.md round 2 のフィードバック履歴、ADR-020 決定 8 に詳細)。

**耳確認の結果 (round 2 の学習修正後に再測定)**: 強く落とすと大きい (peak が J に対して
単調、約 120 倍のダイナミックレンジ) / 面で音が変わる (同一メッシュの複数面で支配周波数が
異なる) / 材質で減衰・周波数が変わる (Wood/Metal/Glass で dom 1480/1121/2336 Hz 前後、
最長減衰 0.07/1.37/0.69 s) の 3 点を実 WAV の数値解析で確認し、絶対音量への上限圧縮は
導入しない判断をした。詳細な実測値は `plans\m76-deepmodal\sub-08.md` の実装メモ
(SELF_EVAL round 2) を参照。

**未実装・後回し (design-draft.md 記載どおり範囲外のまま)**: `D3d11ModalBackend`
(差し込み口のみ)、合成のワーカー化、`ampScale` の -12dBFS 較正 (プレースホルダ 1.0 の
まま — 耳確認では無音/常時クリップのどちらの症状も出なかったため圧縮は不要と判断したが、
論文が意図する絶対較正までは行っていない)、ModelNet10/40 の実走。
