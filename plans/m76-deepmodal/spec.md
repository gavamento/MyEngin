...
| 12 | 音量を絶対 (ピーク正規化しない) にしてよいか | 「強く落とすと大きい」はユーザー計画 Checkpoint J の主張そのもの。正規化すると消える | — | 絶対音量 + `tanh` ソフトクリップ。★**reviewer round 1 で「両方同時に顕在化」が実測され、ユーザー判断で圧縮の導入と `ampScale` の較正を sub-10 で行うことが確定した** (§8)。旧目標「J=1 N·s の中央値ピークが −12 dBFS」は**撤回** — J=1 に根拠が無く、線形則と組み合わせると全域クリップを含意していた。新アンカーは **J = `kImpactRefImpulse` (6.0) で −12 dBFS** |
...
- (sub-05 round 1 時点の暫定値と根拠): CPU 推論 **≤ 6 s/メッシュ**。★旧「≤ 1.5 s」は planner が**何を守る数字か書かずに**置いた値で、実測 4.75–5.34 s に対して「ネットを縮める」判断を促す形になっていた。これが守るべきなのはフレーム時間ではなく**初回起動の体感**である — 焼きは (a) ワーカースレッドで走り (b) メッシュごとに**一度だけ** (2 回目以降は `.msfm` ヒットで実測 0.38 ms) (c) 焼けるまでは従来の音が鳴る段階移行 (§4.1)。エンジンの `assets` は **19 モデル = 382 サブメッシュ**で、初回の裏作業は**実測 wall 183.6 s** (`bakeMsAvg` 453.65 ms、reviewer round 1)。どのフレームも止めない。★旧記述の「100 s 前後」は**モデル数とサブメッシュ数を取り違えた planner の誤り**。
...
| 16 | `Runtime.exe --modal-demo --modal-sync-bake --modal-audio-log 300 --synth-input --screenshot tmp.png --frames 300 > a.txt` を 2 回 → `[modal] t=` 行が byte 一致、`played > 0 && playFailed == 0`。`--no-audio` 併用で 0 行。★**`--screenshot` を必ず付ける** (sub-06 round 1 で判明): これが無いと dt が実時間依存のままで **tick 数が run ごとに揺れる** (実測 25〜110 tick)。`--screenshot` 指定時だけ dt が固定 tick 長になり `frame 番号 == tick 番号` が保証される (CLAUDE.md「決定的撮影モード」)。本サブの変更とは無関係な既存の性質 | 手動 cmd (Release) |
...
| 19 | stage1 (小規模自前 ≤ 100) で `dataset → train → export → --modal-bake → --modal-demo` が端から端まで通り、`assets\deepmodal\deepmodal.dmnet` (≤ 4 MB) をコミット。★**本学習の `R²` (`train.py::compute_r2` のプール定義) を必ず報告し、記録に残すこと** (sub-08 round 1 で漏れた)。`amp_mse` 単体では意味が読めない — **R² ≤ 0 は「平均を返すだけの定数モデルより悪い」** なので、その `.dmnet` をエンジン既定の資産としてコミットしてはいけない。R² が低いまま頭打ちなら学習曲線を添えて報告し planner が裁定する (**同じ `train.py` の設定でユーザーが ModelNet10 を回す**ので、ここで設定の妥当性を確かめておく意味が大きい)。ModelNet10 の手順 (時間見積もり・実行禁止の門・再開方法) が README にある | 実行ログ + README |
| 20 | 文書: `engine_spec.md` §10.7、`docs\adr\ADR-0NN-deep-modal.md` (次の空き番号)、**root `README.md` (日本語の機能概要。`tools\deepmodal\README.md` (手順書) とは別物 — reviewer round 2 指摘 1 で「tools 側だけ直して root を忘れる」が実際に起きた)**、`CLAUDE.md` (末尾 TypeId 61 / Cloth・SoftBody 62/63 / CLI 6 本 / 検証表に `--modal-audio-log` 2 run 一致 / 「.dmnet を差し替えたら --modal-bake」/ constGroups の Python 組) | 目視 + check_rules |
...
| 24 | **衝撃力 → 音量の較正と圧縮** (sub-10、reviewer 指摘 1 + ユーザー判断「今校正する」): 振幅は J に線形なので、`--modal-demo` (J≈10000) が歪まない線形則では **J≈3 N·s (1 kg を 0.5 m 落下) が全サンプル 0** になる。圧縮カーブ `C(J) = kImpactRefImpulse·(J/kImpactRefImpulse)^p` を入れ、`ampScale` を **J = `kImpactRefImpulse` (6.0) で中央値ピーク −12 dBFS ± 3 dB** に較正する。不変条件: **J=3 が全サンプル 0 でなく可聴** / **peak が J に単調増加** (Checkpoint J) / **現実的な力積域 (J = `kImpactMinImpulse` 0.35 〜 100 N·s) の音量差が ≥ 20 dB** ★根拠: エンジンの既存の**波レーン**は `ImpactGain = min(1, J/6)` で J=0.35→6 に **24.7 dB** を割り当てている。モーダルレーンだけが桁違いに平坦だと、同じ衝突なのに「強く当てた」感が波と食い違う (p=0.18 なら 0.35→100 で 8.8 dB、p=0.4 で 19.6 dB、p=0.5 で 24.6 dB) / `--modal-demo` の**登録された impact が全て Played** (`played == impacts` かつ `belowMin` / `dropped` / `poolFull` / `playFailed` が 0) かつ**デモの物体が現実的な質量**であること ★**発音数そのものを条件に書かない** — バウンド回数は質量・反発・`kImpactMinImpulse` の跨ぎ方で変わる派生値で、不変条件ではない (planner が 2 度字面を追随させる羽目になった) ★**デモにカーブを決めさせない** — 1 m³ の `useDensity` 剛体は金属で 7,850 kg になり、その力積 (実測 J = 161〜102164) に合わせて指数を下げると現実域の表現力が犠牲になる。**dBFS × J の表を報告する** (出発値 p=0.5 / `ampScale`≈1.74e4 は合格条件ではなく出発点) | `--modal-wav-dump` の実測 + `--modal-demo` |
| 25 | **無音メッシュの可視化と engine 文書への明記** (sub-10): `ampScale` は mask ゲートの**後**に掛かるので (`ModalSynth.cpp` 66/74/77 行)、**全 mask off のメッシュは較正では救えない** (現行モデルで 382 枚中 35 枚 = 9.2%)。**`--modal-bake` がその数を報告する**こと。engine 文書 (`engine_spec.md §10.7` / `ADR-020` / `README.md`) に (a) 音量則と較正アンカー (b) 全 mask off が出ること・数の出し方・`maskThreshold` の緩和 (c) **ADR-020 決定 5 の訂正** (probe 3 枚の絶対レベルが 2 LSB だった事実) と **決定 8 の採番重複の修正** を書くこと。★**制約が `plans/` にしかない状態を解消するのが本質** | `--modal-bake` の出力 + 文書の目視 |
...
| **sub-10 (M76i)** | **衝撃力 → 音量カーブの較正と圧縮、engine 文書への明記** (reviewer round 1 + ユーザー判断) | sub-08 | 24, 25 | `M76i: 衝撃力に対する音量カーブの較正と圧縮、既知の限界を engine 文書へ` |
...
- 実装中に判明する見込み (coder が「不安・質問」で拾う):
  - 絶対音量に上限圧縮を足すか (**sub-08 の耳確認**で決める。§2 #12。sub-06 round 1 で繰り延べ — 実モデル `.dmnet` が無い段階では乱数重みの音しか出ず聴感評価が成立しないため)
  ...
  - **PhysMat の Save が手書き JSON を再整形する (既知の挙動、M76 の変更ではない。sub-07 round 1 で表面化)**: Inspector の Save (`InspectorWindow.cpp:2305`) と `AssetOps.cpp:447` はどちらも `PhysMatLibrary::ToJson(m).dump(2)` を書くだけなので、押すと `assets\physmats\*.physmat.json` が (a) **キー順がアルファベット順に変わる** (`nlohmann::json` の既定は `std::map` = ソート済み。`ordered_json` ではない) (b) **float が倍精度往復の表記になる** (`0.3` → `0.30000001192092896`。値は float へ戻せば同じだがテキストが変わる) (c) **末尾改行が消える** (`dump(2)` に `\n` を足していない) (d) **既定フィールドが増える** (`acousticSound: ""` 等) という形で再整形される。意味的な値は保たれるので実害は**差分ノイズ**だが、**sub-07 で PhysMat に 4 行足したことで Save を押す動機が増えた**ため、放っておくと 11 本が順次汚れる。
    ★**sub-08 への指示**: α / β の耳合わせは **JSON を手で編集する** (Inspector の Save を押さない)。押すと 11 本のうち触った 1 本だけが別形式になり、M76 のコミットに無関係な再整形が混ざる。
    ★根治 (`ordered_json` 化 / 明示的な float 書式 / 末尾改行) は **M76 の範囲外**。`SoundLibrary` / `MixerLibrary` など同じ `dump(2)` 経路の資産全部に効く変更なので、独立した課題として扱う。
  - **測定値を条件・文書に載せるときは統計量の定義を併記する** (reviewer round 2 指摘 2 と対): 母集団 (何枚の何を) と、**中央値 / 上側中央値 / 平均のどれか**を書く。sub-10 の dBFS 表は「6 面の中央値」と書きながら実装は**上側中央値** (`sorted[3]`) で、一律 **1.78 dB 高い**値を載せていた (合否は変わらないが**第三者が再現できない**)。★R² を報告させた sub-08 と同型の問題 — **数字だけ出して定義を書かないと、読み手が別の量と比べて誤った結論を出す**
  - **`.msfm` の flush は損失の窓を区切る** (reviewer round 2 指摘 4 と対): O(n²) の書き込みを潰すためのバッファリングは可だが、**「終了時に 1 回」にしない**。異常終了で**焼いた結果が全部消える** (ModelNet10 規模では数十分の損失)。モデルファイル単位など、**落ちても失うのが 1 単位で済む**粒度で flush すること
  - **§4.3 の規則 (sub-10 で追加、reviewer 指摘 2 と対)**: **プレビューの既定衝撃力は 1 つの共有定数にする** (Inspector の 4.0 と ヘッドレス `kProbeImpulse` の 15.0 が**双子の定数として 2 箇所にあり片方だけ動いた**)。かつ Inspector は **`Played` 以外の結果を UI に出す** (`BelowMin` / `NotReady` 等。黙って return しない)
  - ★**較正アンカーは 1 メッシュの値で、メッシュ間・面間は正規化されていない (reviewer round 2 指摘 3)**: `ampScale` は **builtin cube 1 枚**の 6 面の**上側中央値**で決めている。同条件 (参照材質・実寸 1 m・`k = C(6)`) に揃えて焼いた `.msfm` 382 枚を比べると、生き残った帯域の振幅和は **p10 0.569 / median 1.925 / p90 14.13** = **p10→p90 で 27.9 dB** 散らばり、**41/382 は cell 中央値が 0**。同じ cube でも **面間で J=6 のとき 16.3 dB** の開きがある (直接測定でも cube(wood, J=21.86) −9.97 dBFS に対し sphere(tile, J=15.21) −5.47 dBFS = √J 補正で約 6 dB 差)。
    ★**この散らばりは一部が「正しい」** — 形状が変われば放射も変わるのは spec §1 の目的そのもの。残りは stage1 のモデル品質 (常時無音 35 枚と同根) で、ModelNet10 で変わる。
    ★**メッシュごとの正規化は意図的にやらない** — やると「形状で音が変わる」(spec §1 の 6 目的のひとつ) を消してしまう。**engine 文書に「アンカーは 1 メッシュ・上側中央値であり、メッシュ間 (約 28 dB) / 面間 (約 16 dB) は正規化されていない」と明記する**のが対処 (ゲームで props ごとに音量が散らばるのは、知らないと不具合に見えるため)
  - ★**stage1 のモデルは弱い接触で無音になる (sub-08 round 2、planner 実測 → sub-10 で較正した)**: `--modal-face-probe` が **4 N·s では 6 面すべて BelowMin**、15 N·s で 3/6 が鳴る。`BelowMin` の実体は力積の門 (`kImpactMinImpulse = 0.35`) ではなく **`BuildModes` の出力が全帯域 `a ≤ kModalTailAmp` に落ちること** = **モデルの絶対振幅が小さい** (`ampScale` が較正前のプレースホルダ 1.0 のまま)。`--modal-demo` は J = 7,650〜102,164 N·s と桁違いに大きいので `played=20/20` になり**この症状が出ない**が、実ゲームの軽い接触 (数 N·s) は無音になる。これは spec §2 #12 が予見した「軽い接触が聞こえない」そのもので、**対処は `ampScale` の較正** (ModelNet10 後へ繰り延べ済み)。★**「弱い接触で鳴らない = 実装の不具合」と誤診しないこと** — 較正待ちの既知の状態である
  - **ModelNet10 の学習では train/val split を入れること** (sub-08 round 2 の指摘)。stage1 の 124 サンプルでは split しても雑音にしかならないので train セットの pooled R² で足りるが、**4899 モデル規模で train R² だけを見ると過学習が隠れる**。論文 §5.1 も train/val/test を分けている
  - **学習は run ごとに再現しない** (sub-08 round 2 実測): cuDNN の非決定的畳み込みで同一コマンドでも R² が ±0.03 程度ぶれ、**`.dmnet` のバイト列も run ごとに変わる**。`.msfm` の再現性 (sub-09) は「**同じ `.dmnet` から**」の保証なので崩れない。厳密な再現が要るなら ModelNet10 の学習で `torch.use_deterministic_algorithms(True)` + seed 固定を検討する
...
- 2026-09-16 (sub-06 round 1): (a) **耳確認を sub-06 → sub-08 へ繰り延べ**。実モデル `assets\deepmodal\deepmodal.dmnet` を作るのは sub-08 なので、sub-06 の時点では乱数重みの `fixture.dmnet` の音しか出せず**聴感評価が原理的に成立しない** (coder の指摘が妥当)。sub-06 は機構レベルの客観証拠 (physmat ごとに f0/f1/modes が変わる / J が小さいほど len が短い) で足りることとし、耳確認と「絶対音量に圧縮を足すか」(§2 #12) の判断は sub-08 の受け入れ条件 0 へ移した。(b) **受け入れ条件 16 のコマンドに `--screenshot` を追加**。`--frames N` だけでは dt が実時間依存のままで **tick 数が run ごとに揺れる** (実測 25〜110)。`--screenshot` 指定時のみ dt が固定 tick 長になり `frame == tick` が保証される (CLAUDE.md「決定的撮影モード」)。M76 の変更とは無関係な既存の性質だが、後続サブとレビュアが同じ罠を踏むので文言を直した (coder の発見)。
- 2026-09-16 (sub-08 round 1、planner 裁定): (a) **`shot_verify` の 2 枚 (acoustic_forward / acoustic_deferred) は pre-existing として記録し、`--update` で塗り潰さない**。coder が git worktree で M76 開始前の基点 `99803eb` と sub-07 の `34bf95f` を両方ビルドし、**同一の失敗 (maxDiff 83/82、diffPixels 596/594、worst pixel 同座標) を再現**したので M76 の変更が原因でないことは立証済み。`--update` を選ばない理由は CLAUDE.md の「golden が動いたときの切り分け」= 原因を断定する前に塗り潰さない、が明示的にあるため。bisect による原因特定は M76 の範囲外の独立課題とする。(b) **`ampScale` の −12 dBFS 較正は ModelNet10 の後へ繰り延べ**。較正の基準がデータセット統計 (J=1 N·s の中央値ピーク) に紐づいており、stage1 のモデルは暫定なので、いま合わせても本学習でやり直しになる。耳確認で無音・常時クリップのどちらも出ていないことも確認済み。(c) **`L_REF` 0.3 → 0.6 を承認** (coverage 0.336 → 0.520、高域 0.829)。0.8–1.2 でさらに伸びる余地を実測しながらグリッドサーチをしなかった判断も妥当 — 1 点動かすたびに stage0/stage1 再生成 + 再学習 + 再 export + `.msfm` 再焼きが要るため。ModelNet10 の前が再訪の自然な機会。`layout.py` / `ModalTypes.h` / **コミットされた `.dmnet` ヘッダ (planner がバイト列を読んで確認: `ref_size_l = 0.6`)** の 3 者が揃っていることを確認済み = σ3 の二重適用や食い違いは無い。
- 2026-09-16 (**ユーザー判断**、reviewer round 1 指摘 1 への回答): **`ampScale` の較正を「今やる」** (sub-08 round 1 の「ModelNet10 の後へ繰り延べ」を**撤回**)。撤回の理由は、繰り延べを決めたときの情報 (peak −28〜−76 dB、無音や常時歪みは無い) が**実態より楽観的だった**こと。reviewer の実測: 振幅は J に線形 (peak/J = Metal 1.150e-6 / Wood 2.41e-6 / Glass 2.79e-6)、int16 の 1 LSB に届くのが J ≈ 11〜27 N·s、**1 kg を 0.5 m 落とす (J≈3) と PCM が全サンプル 0**、`--modal-demo` が鳴るのは箱が 0.7〜7.9 トンだから。**新規サブ sub-10 (M76i) を立てた**。planner の裁定: (a) **線形則では両立しない**ので**圧縮カーブ `C(J) = kImpactRefImpulse·(J/kImpactRefImpulse)^p` を導入する** (J≈3 を可聴にする線形スケールはデモの J≈10000 を約 1000 倍のクリップにする)。基準は新設せず**既存の `acoustic::kImpactRefImpulse` (6.0、「1.5kg を 1m 落下」と文書化済み) を使う** (b) `ampScale` の目標を **J=1 → J=`kImpactRefImpulse` (6.0) で −12 dBFS** へ**再アンカー** (§2 #12 の旧目標は撤回) (c) ★**35 枚の常時無音は較正では直らない** — planner がコードで確認: `ModalSynth.cpp` は 66 行で mask を落とし 74 行で `a<=0` を continue し **77 行で初めて `ampScale` を掛ける**ので、mask 全落ちなら `a=0` のまま。原因はモデルの汎化 (学習 124 形状) で ModelNet10 の仕事。このサブでは**数を報告させて可視化する**だけにする (9.2% が黙って無音なのが本当の欠陥) (d) `.msfm` の**再焼きは不要** (`ampScale` は `weightsHash` を変えないので `modelHash` の照合を通る)。
- 2026-09-16 (reviewer round 1 指摘 7 / 8 / 10 への planner 裁定): (7) `--modal-demo` が 3 個とも Cube で「形状で音が変わる」を見せられない → **`builtin://sphere` を生成順の末尾に 1 個足す** (末尾なら既存 replay / golden に影響しない)。形状依存の機構自体は `.msfm` 382 枚の解析で確認済みなので、見せられるようにするだけ。(8) `.gitattributes` の `*.msfm binary` がどの sub にも割り当てられていなかった (planner の落ち穂) → **1 行足す** (`cache/` にしか生まれないので実害は無いが、prebaked cache を配る等で状況が変わったとき改行変換で静かに壊れるのを防ぐ)。(10) §4.4 の「100 s 前後」はモデル数とサブメッシュ数の取り違え → **19 モデル = 382 サブメッシュ、実測 wall 183.6 s** へ訂正 (結論は不変)。
- 2026-09-17 (sub-10 round 1、planner 裁定): **圧縮指数 p をデモに決めさせない**。coder は p=0.5 のままだと `--modal-demo` が 21/21 フルスケールに張り付くため p=0.18 へ下げたが、planner が `DemoContent.cpp` を読んで**張り付きの原因はデモ側**と判定 — 落下する箱は `SetLocalScale(1,1,1)` + `useDensity = true` なので **1 m³ の中実剛体 = 金属で 7,850 kg**、力積が J = 161〜102164 N·s になる。これは「衝突音のショーケース」として非現実的で、**その力積に合わせて指数を下げると現実的な衝突 (J = 0.35〜100) の表現力が犠牲になる**。実測: p=0.18 では現実域の音量差が **8.8 dB** しかなく、**エンジンの既存の波レーン (`ImpactGain = min(1, J/6)`) が J=0.35→6 に割り当てている 24.7 dB と比べて桁違いに平坦** = 同じ衝突なのに波と modal で「強く当てた」感が食い違う。よって受け入れ条件 24 に「現実域の音量差 ≥ 20 dB」と「デモの物体は現実的な質量」を追加した。アンカー (J=6 で −12 dBFS) が p 非依存の恒等式であることは coder の指摘どおり正しい。
- 2026-09-17 (sub-10 round 3、確定): **`kModalImpulseExponent = 0.5` / `ampScale = 11478` (不変) で確定**。実測が p=0.5 の冪則と **0.01 dB で一致** (J=0.35 → −24.34 / 1 → −19.78 / 3 → −15.01 / 6 → −12.00 / 15 → −8.02 / 30 → −5.01、J=100 だけリミッタで −0.34)。現実域の音量差 **24.0 dB** (理論 24.6、波レーン 24.7)。デモは `useDensity` をやめ `mass` 直指定 (木 2 / 金属 4 / ガラス 1 / 球 1.5 kg) にして J = 0.438〜52.058 に収め、張り付き 0 発。★**planner の訂正**: round 2 の申し送りで「p 変更後は `ampScale` の再計算・再 export が要る」と書いたのは**誤り**。`C(R) = R·(R/R)^p = R` はどの p でも成り立つので**アンカーは p 非依存**であり、`ampScale` は据え置きが正しい (coder が round 1 から一貫して正しく、round 3 で「p=0.18 と p=0.5 で J=6 の実測が同一」という形で直接実証した)。★**受け入れ条件 24 から発音数の固定値を削除**した — バウンド回数は質量・反発・`kImpactMinImpulse` の跨ぎ方で決まる派生値で不変条件ではない (planner が 21/21 → 20/20 と 2 度字面を追随させる羽目になった記録)。
- 2026-09-17 (reviewer round 2 への応答): 指摘 3 (planner 宛) を**認める**。`ampScale` のアンカーは builtin cube 1 枚の 6 面の上側中央値で決めており、**アセット全体を代表していない** (382 枚で p10→p90 が 27.9 dB、41 枚は cell 中央値 0、同じ cube の面間でも 16.3 dB)。planner が reviewer の数値を検算して再現 (√J 補正後の cube/sphere 差 = 6.08 dB、p10→p90 = 27.9 dB)。裁定: **メッシュごとの正規化はしない** — それをやると「形状で音が変わる」(spec §1) を消してしまうため。散らばりの一部は物理的に正しく、残りは stage1 のモデル品質 (常時無音 35 枚と同根、ModelNet10 で変わる)。対処は **engine 文書への明記** (§7 に記録、`engine_spec.md §10.7.2` に 1〜2 文)。あわせて coder 宛の指摘から仕様側の規則を 3 つ足した: (2) **統計量の定義を併記する** (「中央値」と書いて上側中央値を出していた = 再現不能。R² を報告させた sub-08 と同型) / (4) **`.msfm` の flush は損失の窓を区切る** (終了時 1 回だと異常終了で全部消える) / (1) 受け入れ条件 20 の `README.md` を **root と `tools\deepmodal\` の別物**と明示 (tools 側だけ直して root を忘れる事故が実際に起きた)。
