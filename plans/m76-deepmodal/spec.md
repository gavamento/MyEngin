...
- 実装中に判明する見込み (coder が「不安・質問」で拾う):
  ...
  - ★**stage1 のモデルは弱い接触で無音になる (sub-08 round 2、planner 実測 → sub-10 で較正する)**: `--modal-face-probe` が **4 N·s では 6 面すべて BelowMin**、15 N·s で 3/6 が鳴る。`BelowMin` の実体は力積の門 (`kImpactMinImpulse = 0.35`) ではなく **`BuildModes` の出力が全帯域 `a ≤ kModalTailAmp` に落ちること** = **モデルの絶対振幅が小さい** (`ampScale` が較正前のプレースホルダ 1.0 のまま)。`--modal-demo` は J = 7,650〜102,164 N·s と桁違いに大きいので `played=20/20` になり**この症状が出ない**が、実ゲームの軽い接触 (数 N·s) は無音になる。これは spec §2 #12 が予見した「軽い接触が聞こえない」そのもので、**対処は `ampScale` の較正** (ModelNet10 後へ繰り延べ済み)。★**「弱い接触で鳴らない = 実装の不具合」と誤診しないこと** — 較正待ちの既知の状態である
