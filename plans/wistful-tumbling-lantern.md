...
| M52i ロールバック + ABI v13 | 完了 | (本コミット) | 下記「M52i の申し送り」参照 |
...
1. **`net.OnTickConsumed` を投機 tick で進めてはいけない。** ここがロールバック実装で
   最初に踏む罠だった。`NetSession` は「消費済み tick より前の入力は捨てる」でリングを
   守っているので、予測で走った tick まで消費済みにすると**後から届いた本物の入力が
   その場で捨てられ、予測が永久に訂正されない**。しかもハッシュは (両者が対称に
   間違えない限り) 割れるので、症状は「たまに desync する」という一番追いづらい形で出る。
   進めてよいのは確定フロンティアまで (`NetCommitConfirmed` の中の 1 行)。

2. **予測 tick を .rep に書かない = 記録を `RunOneTick` から追い出す、が中心的な設計判断。**
   書いてしまうと巻き戻して走り直した tick が二重に並び、**このサブ唯一の機械検証
   (2 台の .rep 一致 + ローカル 2P 参照とも一致) が成立しなくなる**。確定した tick だけを
   追記する形にすると、ロールバック有りの .rep はロックステップのものとバイト一致する。
   ★ロックステップ経路 (`--net-no-rollback`) は M52h のまま `RunOneTick` 内で記録している。
   経路を 1 本に畳まなかったのは、既に緑だった M52h の .rep 生成を触らないため。

3. **desync の checkpoint を固定刻みにしないと、2 台が別々の tick を報告する。**
   最初の実装は「相手が最後に主張した (tick, hash) を毎フレーム比べる」で、実測で
   片方が tick 60、もう片方が tick 68 を desync とした (比較対象が到着タイミングで決まる)。
   バンドルのフォルダ名が食い違うと診断の第一歩 (`--rep-diff` に 2 本渡す) に進めない。
   **8 tick 刻み (`kNetHashCheckpoint`) の確定ハッシュだけを主張する**形にして解決。
   厳密な初発 tick は `--rep-diff` が出す (実測: poke した tick 60 をそのまま名指し)。
   ★主張パケットがロスで落ちたときのために「相手が 4 checkpoint 以上先へ行っていたら
   その checkpoint は諦めて次へ」の逃げ道が要る (無いと検出が永久に止まる)。

4. **バンドルのフォルダ名にレーン番号を入れる。** 同一マシンで 2 プロセスを回すと
   (net_verify がまさにそれ) 同名フォルダを両者が上書きし合って、突き合わせに要る
   2 本目が残らない。`desync_<tick>_p<lane>`。

5. **`CrashRing::Rewind` が要る (M52f の穴の回収)。** ロールバックのたびに
   `OnTickBegin` が「tick 列が飛んだ」と判定して撮り直すので、ネット対戦中の
   crash.rep / desync バンドルの .rep が**常に 1〜2 tick しか持たない**状態だった (実測)。
   保持スナップショット以降なら、レコード本数を切り詰めるだけで整合する。

6. **`--rep-diff` は長さ違いを門前払いにしてはいけない。** desync バンドルの 2 本は
   「割れた側が先に気づいて先に止まる」ので**必ず長さが違う**。共通部分を比べてから
   長さの違いを結論として述べる形に変えた (`same` は立てないので検証としては失敗のまま)。

7. **`--net-poke-tick` は tick 本体の中 (構造変更適用の直後 = ハッシュもダンプも撮る前) に
   置く。** tick の外から壊すと記録ハッシュに載らず「再生では再現しない変異」になり、
   `--hash-dump` より後ろだと今度は診断ダンプにだけ映らない。ここに置けば
   record / verify / ネット / `--hash-dump` の 4 経路が同じ 1 フィールドの変異を見る。

8. **tick 末ハッシュを 1 本に畳んだ** (M52f 申し送り 6 の回収)。クラッシュリング /
   タイムトラベル / ロールバックが同じ 1 個を使う (`TickEndHash`)。`TimeTravel::OnTickEnd`
   はハッシュを引数で受け取る形へ変更し、`HashOf` を public にして「同じ 3 出口・
   同じ引数で撮った値であること」を型で示せるようにした。

9. **タイムトラベルはネット中は起こさない。** 相手の居る tick 列を勝手に巻き戻しても
   相手はついてこないので、スクラブは原理的に成立しない。同じ tick 境界を 2 種類の
   巻き戻しが奪い合う状態も避けられる。機能の削りではなく意味論の帰結。

10. **実測** (`--local-demo` 7 エンティティ、300 tick、ループバック):
    ケース A (Debug↔Debug / ロス 0% / 遅延 3) ホスト 7 回巻き戻し / 再シム 14 tick /
    最大深さ 5 / 予測 124 tick。ケース C (Debug↔Release / ロス 30% / **遅延 1**)
    ジョイナー 21 回 / 54 tick / 深さ 4 / 予測 269 tick。**いずれも .rep はローカル 2P
    参照とバイト一致**。巻き戻し 1 回あたり約 2ms (Debug + WARP、深さ 5)。
    ★ロールバックが「起きたこと」自体も bat で要求している (`findstr` で統計行を確認) —
    一致しただけでは 1 度も巻き戻していない可能性が残るため。

11. **desync 注入の実地検証** (net_verify ケース E): poke tick 60 → 両者が checkpoint 64 で
    検出 → 両者 exit 4 + `desync_64_p0` / `desync_64_p1` → `--rep-diff` が **tick 60** を
    名指し → 2 本を tick 60 で `--hash-dump` して `--hash-diff` が
    **`Main Camera LocalTransform.position` の 1 フィールドだけ**を出す。
    M52a の診断チェーンと M52i の検出器が通しで繋がることの実証。

12. **ABI v13 = 94 スロット** (7 本 append)。`Net*` 5 本は**機種依存の値を返す唯一の
    スロット群**で、sim へ書き戻すと必ず割れる。型でも API でも防げないので
    「禁止しない・必ず検出する」を選んだ (desync 検出とセットで 1 つの設計)。
    `--net-demo` の `NetHudDemo` (UIElement = NoHash にしか書かない) と `NetDuelDemo`
    (レーン入力 = 決定論の内側) を使い分けの実例として同梱。
    ★`REGISTER_SCRIPT(T, FIELDS())` は**空配列でコンパイルエラー**。フィールド 0 本の
    スクリプトは書けない (`NetHudDemo` に設定用の `style` を 1 本置いた)。

13. **`.bat` の `if (...)` ブロック内の `echo` に素の `)` を書くとブロックが早期終了する。**
    `echo ... expected 4 (desync)` が原因で、条件が偽なのに直後の
    `set /a FAILED+=1` と `goto :eof` が**無条件実行**され、「何も表示されないのに
    1 件失敗」という読めない壊れ方をした。`^(` `^)` で逃がす。
    ★`findstr /c:"predicted tick(s)"` のように**引用符の中**なら影響しない。

14. **未検証**: ①実 LAN / 物理 2 台 (ここまで全部ループバック) ②パケット並べ替え
    ③長時間セッション (最長 300 tick) ④実パッドでのネット対戦の目視
    ⑤エディタの NetWindow の実機目視 (自動化不能。ユーザー同席時に)
    ⑥`--net-demo` の見た目の目視 (ハッシュ上の動作確認と 900 tick の得点発生までは実走済み)。
