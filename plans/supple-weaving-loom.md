...
- TypeId は 43 (Vehicle) まで消費済み → **44=Rope / 51=Cloth / 52=SoftBody** (登録順 =
  実装順 c → h → k を厳守)。
...
- 予約事項 6 (TypeId 末尾 append) → 44/51/52 (M65a が 45〜49、M68a が 50 を取った後の番号)。
...
### M60'h: 布 — Cloth (TypeId 51) + 曲げ拘束
- `ClothComponent` (TypeId 51): `countX` / `countY` / `width` / `height` / `mass` /
...
### M60'k: ソフトボディ — SoftBody (TypeId 52) + 体積拘束
- `SoftBodyComponent` (TypeId 52): 閉表面メッシュ (球/箱のプリセット生成。任意メッシュは
...
| TypeId | c, h, k | **44=Rope / 51=Cloth / 52=SoftBody** を末尾 append (45〜49 は M65a の音響 5 本、50 は M68a の AcousticAudio) |
...
| M60'b XpbdBackend 骨格 | 完了 | `85a9cdd` | 下記「M60'b の申し送り」参照 |
| M60'c ソルバ核 + Rope | 完了 | (本コミット) | M60b 申し送り 8 も消化。下記「M60'c の申し送り」参照 |
| M60'd アタッチ双方向 | 完了 | (本コミット) | 冒頭判定 = **ConstraintBlock の K は流用不可** (速度レベル + 方向/K⁻¹ を生成時固定 + 解かれる場所が剛体速度ソルバ内で粒子が居ない) → Macklin 式 w(n) = invM + (r×n)·I⁻¹(r×n) で新設 (式自体は FinalizeConstraintBlock の対角項と同じ。腕 r 反復中固定も「K は生成時 1 回」の踏襲)。下記「M60'd の申し送り」参照 |
...
### M60'c の申し送り (計画外の事実・罠)

1. ★**`PhysicsSystem::Update` の早期 return (`bodies.empty() && chars.empty()`) が
   剛体ゼロのロープシーンで substep ごと飛ばしていた** — b の「Sync を先頭に置く」
   だけでは足りず、「池が生きていれば回る」を条件に足した。存在ゲートの同値性は
   維持 (池が空なら従来と同条件)。
2. **減衰は定常状態の実効重力を (1−damping) 倍にする** — 減衰が「重力を足した直後の
   速度」に掛かるため。吊り下げの静的伸びの解析値は α·m·g·(1−damping) で、実測と
   <0.1% で一致 (Rigidbody.linearDamping と同じ意味論。テストの期待式に注意)。
3. **鎖の先端は自由落下上限 √(2gL) を鞭効果で正当に超える** (L=4m で実測 13.2 m/s、
   質点の上限は 8.9)。エネルギー検査の堰は 15 m/s に置いた — 発散検知には十分で、
   決定論なので値はビットまで再現する。
4. **M60b 申し送り 8 (Joint Inspector の絵確認) 消化** — 全 5 型の type コンボと
   出し分けを絵で確認、仕様どおり (Ball=線形のみ / Hinge・Slider=軸+リミット+モータ /
   Fixed=破断のみ / Cone=swingLimitDeg あり・モータ無し)。**レシピ**: エディタ UI は
   `--screenshot` が ImGui 込みの backbuffer を保存するので、`--select NAME` (既存
   フラグ) + `--width/--height` で撮れる。Inspector が縦に収まらないフル構成の
   エンティティは、一時プローブで「対象コンポーネント単体のエンティティを作って
   選択」するのが最安 (今回は env var ゲートで足して撮影後に除去した)。
5. **replay_verify 実行中に Editor.exe を並走させない** — 8 ビルド工程が exe を
   リンクし直すためロックで落ちる (LNK1168)。今回は並走してしまったので、完走後に
   クリーンツリーで録り直して合格を取り直した。
6. 剛体収集の親フレーム合成を `ComposeEntityWorldPose` として公開 (PhysicsSystem.h)。
   Sync の初期配置とピン追従が剛体と同じ式を通る = 正本 1 つ。

### M60'd の申し送り (計画外の事実・罠)

1. **アンカーは予測姿勢 (位置積分の先取り) で組み、補正は速度へだけ返す** — XPBD Solve は
   剛体の位置積分の**前段**にあるので、現在姿勢で組むと連成が 1 substep 遅れ、姿勢へ直接
   書くと後段の位置積分 (補正済み速度で動く) と二重適用になる。予測式は位置積分 (4263-)
   と同式のミラー。眠り/静的/kinematic は invMass=0 ゲートで「予測 = 現在」に自然に落ちる。
2. **焼き込みローカルアンカー (COM 系) は新 sim 状態** — attachValid + attachLx/Ly/Lz を
   Pool へ追加、3 点セット (hash / snapshot) 対応で kSimSnapshotVersion 5→6。毎 tick
   引き直すとドリフト + 「いつ焼いたか」依存で復元後の再シムが割れる。相手が消えたら
   attachValid=0 → 繋ぎ直しでその時点の位置関係を焼き直す (= 切り離しの自由落下も自動)。
3. ★**眠った剛体の起床シグナルは「違反量」では作れない** — アタッチ行は各反復の末尾に
   置いた (鎖の固定順) ので、粒子が毎反復アンカーへ吸い付き、反復前に測っても違反が
   残らない。信号は「アタッチ行が粒子側へ実際に適用した補正の総和 Σ(w_p·|dλ|)」
   (AttachContext::outAbsCorr)。しきい値 5mm は substeps=1 の自由落下予測 g·h² ≈ 2.7mm
   より上 (下げると吊るしただけで眠りと起床が往復する)。
4. **テストの罠 3 つ**: ①粒子と剛体の逆質量比を数百:1 にしない — Gauss-Seidel の伝播が
   tick あたり数 % に落ち、未収束ラグが「余計な定常伸び」に化ける (0.02kg vs 5kg で解析値の
   4 倍を実測。25:1 なら 0.2% で一致)。②substeps>1 の実効重力は (1 − damping/substeps) —
   減衰が噛むのは sub0 の残速度 = 1 substep 分の重力だけ (c の (1−d) は substeps=1 の
   特殊形)。③起床判定は引いた**直後** (5 tick) に見る — 引き上げが静定すると再入眠が
   正しく判定を上書きする。
5. トルク経路 (実効質量の逆慣性項 + outT) の実証はアンカーオフセット検査で取った:
   中心を 0.4m 外して吊った箱が |q.xyz|=0.615 まで回転し、アンカーが x=0.001 の真下静止。
6. replay 被覆はまだセルフテストのみ (デモシーンに Rope が無い)。m (ショーケース) で
   6 ペア側へ載せるまでは、replay_verify の「無風 PASS」が既存シーン不変の証明。
