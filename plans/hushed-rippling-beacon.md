...
| M65f 流れ場 + FSM | **完了** | `AcousticNav` / `AgentSystem` / `Advance(World*, tick)` / golden 2 枚更新 | 下の「M65f の申し送り」参照 |
| M65g ゲームの薄皮 | **完了** | **ABI v15 据え置き** / `Watcher*` 3 本 / 光は使い回し / replay 記録側に `--synth-input` / golden 2 枚更新 | 下の「M65g の申し送り」参照 |
...
## M65b の申し送り (計画外の事実・罠だけ)

**実測**: 両構成 0 警告 / selftest 43 本 ALL PASS (音響は 12 群) / replay_verify **10 ジョブ PASS
(7 ペア目 = acoustic)** / shot_verify **17 枚すべて maxDiff=0** / check_rules 0 error /
build_managed Debug+Release 0。

1. ★★**デモの音響ボリュームを壁より高くすると波が壁を飛び越える**。最初 52x8x52 (y[-0.5,3.5])
   で組んだら、壁 (y[0,3]) の上に 0.5m の開セル層が残り、**波が L 字を曲がらずに直接となりの
   部屋へ届いた**。到達セル数が開セルのほぼ全部 (10860/10864) になり、企画の中核が絵から消える。
   52x6x52 / 中心 y=1.75 (y[0.25,3.25]) にして壁が全 6 層を塞ぐ形に直した。
   **水平は緩く包み、垂直はきつく取る**のが正しい。この種の穴は
   「到達セル数」と「リング毎のシェルのセル数」を出す一時プローブでしか見つからない
   (実測: 直後は corridor 断面 = **24 セル**ちょうどのシェルが 20 リング続く = 廊下を平面波が
   進んでいる証拠になる)。
2. ★**斜めの角抜けを許す実装にした**。凸角を 11+11=22 ではなく 16 で曲がれる。理由は
   (a) 回折らしく見える (b) 内側ループの IsSolid が 26 回で済む (中間セル判定を足すと最大 74 回
   = 伝播の主コストが 3 倍)。代償は「斜め 1 セル厚の壁」が漏れること — 壁は box で組む前提なので
   ボクセル化すると必ず軸平行 1 セル厚以上になり実際には起きない。selftest (8) が角抜けの
   コスト (34*11+16) を、(8b) が軸平行 1 セル厚で漏れないことを固定している。
3. **`kMaxWaveRing` を 40 → 64 に上げた**。40 (= 20m) では 24x24m の間取りを渡れない。
   メモリはボックスがグリッドでクリップされるので既定ボリュームで 196KB/波 (16 本で 3.1MB) —
   計画の「81^3 = 1.6MB/波」はクリップを見落とした過大見積りだった。
4. `SimSources` は M65a の申し送り 2 のとおり 6 箇所。M65b では触っていない。
5. **リングの幅を面コスト (11) ちょうどにしたのが Dial 法が成立する条件**。relax の挿入先は
   必ず d+11 以上 = 現区間の外なので、処理済みバケットへ書き戻されることが構造的に起きない。
   幅を変えるならこの性質を先に確かめること。
6. **シーン構築直後の 1 tick だけ占有を 2 回焼く**。フェーズ 3.4 は TransformSystem (4) より
   前なので、最初の tick は全コライダの WorldMatrix が既定値 = 全部原点に重なった署名になる。
   決定論的な 2 回なので害は無い (実測: 4 セル → 8064 セル)。
7. **占有デバッグ線はセル中心に描くので、壁の内部にあるぶんはジオメトリに隠れる**。
   グリッド天面が壁の天面と一致していると壁の輪郭が綺麗に出る (今のデモはそうなっている)。
   隠れて困るときは SceneView を Wireframe / Unlit にする。
8. ★**M65g の冒頭確認 1 は先に片付いた**。C++ スクリプトから `MyeSetField` で組込み
   コンポーネントの sim 状態フィールドを書けることを `WavePinger` が実走で示した
   (`VehicleDemoDriver` と同じ形)。**ABI v15 据え置きが確定**。
9. スクリプト `WavePinger` を足した。名前順ソートで TypeId が決まるので現行末尾の
   `WalkerDemo` の後に入る。M65g の `Watcher*` 3 本は `WalkerDemo` と `WavePinger` の**間**に
   入って WavePinger の TypeId を 3 つずらすが、使うのはコードから毎回組み直す音響デモだけで、
   `golden_acoustic.rep` は replay_verify が毎回録り直すので**永続的な破壊は無い**。
10. `AcousticEmitterComponent.cooldownTicks` の意味を「発音の**後**に置くクールダウン」で
    実装した (要求はクールダウン中なら**捨てる**。溜め込むと「いつ鳴るか」が過去の混雑に
    依存する)。`autoFootstep` / `stepDistanceM` / `travelAccum` は M65c まで未使用のまま。
11. デモの材質は**真っ黒にしていない** (floor 0.13 / wall 0.20 + 環境光 0.35)。企画は真っ暗だが、
    M65e で golden を撮るときに全画素が黒だと**機能が壊れても golden 一致で通る**ため
    (計画 M65e の最大の罠)。この配色は M65e でそのまま使う前提。
---

## M65c の申し送り (計画外の事実・罠だけ)

**実測**: 両構成 0 警告 / selftest 43 本 ALL PASS (音響は 15 群 / PhysMat も拡張) /
replay_verify **10 ジョブ PASS (100.3s)** / shot_verify **17 枚すべて maxDiff=0** /
check_rules 0 error / build_managed Debug+Release 0。

1. ★**`wood` は既存プリセットと名前が衝突していた**。計画は「carpet/wood/gravel/water/metal/glass
   を 6 種**新規**、既存 5 種は触らない」と書いていたが、`assets\physmats\wood.physmat.json` は
   M59a1 から在って `--joint-demo` が使っている。`wood_floor` を別に作ると木の物理値が
   二重管理になるので、**既存 wood に音響 3 キーを追記**し、新規は 5 種にした。物理値は
   1 バイトも変えていない。joint デモには AcousticVolume が無いので存在ゲートで無音のまま。
2. ★**上書きビット (`kPhysMatOverrideAcoustic`) は足さなかった**。計画は `Components.h` の
   末尾へ足すと書いていたが、`SelectAdhesion` (M60d) の前例どおり「Collider 側に旧フィールドが
   無い = 材料が無ければ無音しか状態が無い」ので、ビットを足すと「黙らせる手段が 2 つ」に
   なるだけ。黙らせたい床は材料を外す。
3. ★★**反発 0.5 の落下箱は永久音源になる**。最初 `restitution=0.5` で組んだら、跳ね返りが
   減衰しきらず **5 tick 周期・力積 1.6 N*s のリミットサイクル**に入り、900 tick で
   **衝撃音を 151 回**出し続けた。反発 + 貫通押し出しの古典的なエネルギー注入で、速度が
   0.5 m/s あるのでスリープ閾値 (0.05 m/s) にも掛からない。デモは反発 0 に落として 1 発だけに
   した (減衰そのものは `ImpactGain` のセルフテストが固定している)。
   ★**この種の穴は絵では絶対に見えない** — `Emit` に一時ログを入れて「いつ・どの材質で・
   どれだけの振幅の波が立ったか」の一覧を出すのが唯一の見つけ方。M65b の「到達セル数」と
   同じ教訓で、**音響のバグはスクショではなく発音の数列で捕まえる**。
4. **「載っているだけ」の力積の差し引きは必須**。素の閾値だと (3) と無関係に、重い箱が静止して
   いるだけで鳴り続ける。引く量は `質量 x g x dt x kImpactRestingMargin(2)`。g は
   `PhysicsEnvironment` から取る — 重力を強くしたシーンで 9.81 固定にすると同じ穴が開く。
5. ★**床材タイルの厚みには 2 つの制約が同時に掛かる**。
   (a) 天面が音響ボリュームの下端 (デモでは y=0.25) より**上**でないと、衝撃の接触点が
   `WorldToCell` で外れて「材質は正しいのに 1 本も鳴らない」という理由の分かりにくい沈黙になる。
   (b) セル中心 (層 0 = y=0.5) には**届かない**厚みでないと廊下の床が占有セルになり、
   M65b で測った「断面 24 セルの平面波」が変わる。
   デモは 厚み 0.6 / 中心 y=0.15 (天面 0.45) の一択。
6. **タイルは隙間なく並べる**。0.45m の段差は CC の collide-and-slide では登れず、歩行者は
   そこで止まる (足音の被写体が丸ごと死ぬ)。
7. 足音の床材は `RaycastWorld` で真下を見る。★**CC と同じエンティティにソリッド Collider を
   併用すると自分に当たって無音**になる (自己ヒットは無音扱いで落としてある)。足音の要る
   キャラは CC 単体で組むこと。
8. `--acoustic-demo` の発音実測 (900 tick): WavePinger 6 / 足音 32 / 衝撃 1。足音は
   carpet 0.12/6 リング → wood 0.3/14 → gravel 0.55/26 → water 0.8/40 → metal 1.0/60 →
   glass 1.0/56 と往復する。**企画 3-4 の表がそのまま数列として出る**のが、この機能が
   繋がっている証拠になる。
9. スクリプト `WaveWalker` を足した。`WavePinger` の後ろ (WaveP < WaveW) なので既存スクリプトの
   TypeId は 1 つも動かない。M65g の `Watcher*` は依然 `WalkerDemo` と `WavePinger` の間。
10. `.physmat.json.meta` の GUID は**初回走査時のパスハッシュが凍結される**もの。新規 5 種は
    一度実行して生成された .meta をそのままコミットしてある (既存 5 種と同じ流儀)。
    ★**コミットし忘れると各人の checkout パスで GUID が変わり**、`Collider.physMaterial`
    (ハッシュ対象) が機種依存になる。
...
## M65f の申し送り (計画外の事実・罠だけ)

**実測**: 両構成 0 警告 / selftest 43 本 ALL PASS (音響は 23 群) /
replay_verify **10 ジョブ PASS (79.8s)** — 敵 2 体が動くシーンでハッシュ列が一致する =
**AI の決定論の唯一の証明** / shot_verify 19 枚 PASS (音響 2 枚は更新、他 17 枚は maxDiff=0) /
check_rules 0 error / build_managed Debug+Release 0。

1. ★★**タイルの段差 0.45m が M65f のバグの正体だった**。M65c は床材タイルを
   厚み 0.6 / 天面 0.45 で置いていたが、追跡中の敵がタイルの端 (x=4.2) で
   **永久に足踏み**した (probe で発見: `v(-3.00,0.00)` なのに `pos` が動かない)。
   collide-and-slide は 0.45m を登れない。M65c の申し送り 6 が
   「0.45m の段差は登れない」と書いていたのに、歩行者はタイルの上で生まれるので
   一度も跨がず、**絵にも replay にも出ていなかった**。天面 0.05 (中心 -0.25) へ下げた。
   ★あわせて M65c の制約の読み直し: 「天面が音響ボリュームの下端より上」は
   **タイルには要らない** (足音は歩行者の位置で鳴る)。効くのは接触点がそのまま発音位置に
   なる**衝撃の金属板だけ**で、あちらは 0.45 のまま。
2. ★★**tick 0 では「まだ一度も聞いていない」と「今 tick に聞いた」が区別できない**。
   鏡の `lastHeardTick` は 0 が「未聴」なので、`lastHeardTick == tick` だけを見ると
   tick 0 で全個体が警戒に入る (実際に入った)。`!= 0` を必ず先に見ること。
3. ★★**閾値は逆二乗のスケールで決める**。`EnergyAt` は minD = 1 セル (0.5m) の逆二乗なので、
   振幅 1.0 の音でも **10m 先では (0.5/10)^2 = 0.0025** しかない。最初 0.015 という
   「一見ちょうどよさそうな」値を入れたら 600 tick 走らせて 1 度も反応しなかった。
   デモは 0.0015 = 廊下 (10m) の water/metal/glass は聞こえて gravel は聞こえない境。
4. ★**到達を配るのは内側 (バケット) のループではなく波ごとのループ**にした。計画本文は
   「リング前進のループ内で」と書いていたが、内側では (a) World を知らない
   (b) `Rebuild()` がリングを引き直すので**復元のたびに過去の到達が今の tick で再通知される**。
   食い違わない根拠は「同じ `dist` 配列・同じ `EnergyAt`」であって「同じ最内ループ」ではない。
5. ★**流れ場のバケットは毎回 clear しない**。`pending` (未処理エントリ数) で早期終了させ、
   「pending==0 なら全バケットが空」という不変条件に乗せてある。素直に
   `assign` すると 2 万本の vector を毎 tick 作り直すことになり、流れ場のコストの
   大半がそこになる。
6. ★**粗グリッドの「過半が開なら開」は廊下の幅に効く**。selftest の迷路は
   **1 セル幅では粗グリッドで廊下が丸ごと閉じる**ので、`MakeLMaze2` は 2 セル幅にしてある。
   デモの廊下は 2m = 4 セルなので問題ない。1m 幅の通路を作るなら navCellRatio を 1 にすること。
7. ★**`AgentBrain` に「初期化済み」フラグを足さなかった**。TypeId が確定済みでフィールドを
   足すと sizeof / ハッシュ / シリアライズが動くので、「home が厳密に原点なら未設定」で
   代用している。デモは `br->home` を明示的に焼いてあるので、この経路には乗らない。
8. **巡回は home の周りを歩く** (`kPatrolRepathTicks` 120 / 半径 4m)。目標を home に固定すると
   着いた瞬間に立ち止まって二度と動かない = 「巡回」という名前だけが残る。
   乱数を引くのは**巡回と探索の目標づけだけ**で、どちらもエージェントが居るときにしか走らない。
9. **警戒中に鳴らないことの検査は「波の本数の増分」では不十分**。同じ tick に別の波が
   寿命で消えると増分が打ち消される。`source` と `bornTick` で数える
   (`CountWavesFrom`) 形に直した。
10. `AgentSystem` は **3 点セット契約の対象外**。ECS 外に持つのは航法グリッド (導出値) だけで、
    FSM の状態は全部 `AgentBrainComponent` = ECS = snapshot に載っている。
    `XpbdBackend` / `AcousticField` とはそこが違う。
11. **敵にソリッド Collider を付けないこと**。(a) 足音のレイが自分に当たる
    (b) 動くコライダは占有ベイクの署名を毎 tick 変えて全再ベイクが走る。CC 単体で組む。
12. デモの実測 (600 tick): 耳の敵は t=84 に廊下の足音を聞き → 警戒 30 tick → 追跡で
    **部屋 B から縦の廊下を通って横の廊下へ**降りてくる。目の敵は t=0 に設置光を見つけ、
    同じ L 字を辿って部屋 A の光へ着く。**「聞こえた経路と同じ道を通って寄ってくる」が
    座標の数列として出る**。
13. M65g への申し送り: 敵の `lastTone` は鏡に載っているので「何の音か」で反応を変える余地が
    ある (v1 の FSM は使っていない)。視線判定は**無い** — 目の敵は壁の向こうの光にも引かれる。

---

## M65g の申し送り (計画外の事実・罠だけ)

**実測**: 両構成 0 警告 / selftest 43 本 ALL PASS / replay_verify **10 ジョブ PASS (84.3s)** —
7 ペア目はプレイヤーが `--synth-input` で歩き回りながら一致する / shot_verify 19 枚 PASS
(音響 2 枚は更新、**他 17 枚は `--update` してもバイト同一**) / check_rules 0 error /
build_managed Debug+Release 0。**ABI は v15 = 104 スロットのまま** (冒頭確認 1 は M65b で決着済み)。

1. ★★**スクリプトから足したコンポーネントは tick 末まで存在しない**。EngineAPI.h v14 に
   明文で書いてあるのに踏んだ。`CreateGameObject` → `AddComponentByName("Rigidbody")` →
   `SetVelocity` と続けて書くと **SetVelocity が 0 を返し**、石は投げた場所へぽとりと
   落ちるだけになる (probe で `ok=0` と z が 1cm も動かない数列を見て初めて分かった)。
   投擲は「生成の次の tick に `Arm()` で質量・反発・射出速度を書く」形に直した。
   ★**光では同じ手が使えない**。1 tick とはいえ `LightComponent` が既定値で実在する
   フレームが挟まり、**白 / intensity 1.0 の平行光**がシーン全体を照らす = 暗闇のゲームで
   最悪の閃光になる。だから光は**シーンが 3 個を床下に用意して使い回す**形にした
   (構造変更ゼロ)。企画 4-1 の「同時に持てるのは最大 3 本」がそのまま固定プールなので、
   設計としてもこちらが素直。
2. ★★**登録フィールドは 1 スクリプト 16 本まで** (`ScriptAPI.h` の `MYE_SF_FOREACH`)。
   20 本書いたら `MYE_SF_prevViewKey` が見つからないという読み解けないエラーで落ちた。
   上限は窮屈に見えるが「動かないものを ECS に置かない」線引きを強制してくれる —
   ★**登録しなかったメンバは snapshot にもハッシュにも載らない**ので、sim に効く値を
   非登録で持つと**タイムトラベルとロールバックでだけ壊れる**。調整値は無名名前空間の
   `constexpr` へ、状態だけをフィールドへ、が正しい分け方。
3. ★**プレハブ資産にしなかった** (計画本文は `.prefab.json` 3 種と書いていた)。
   `PlayEffect` / `Instantiate` は prefabKey を assets ルートで絶対化して
   **正規化パスのハッシュ**を引き、それが `PrefabInstance.prefabHash` = ハッシュ対象
   フィールドに載る = sim 状態がチェックアウト先に依存する。`Spawner.cpp` が M13 から
   使っている「CreateGameObject + SetMeshRenderer("builtin://...") +
   AddComponentByName」にはパス由来の値が 1 つも無い。
4. ★★**CC の寸法にはスケールが掛かる**。`height 1.6` のキャラに `scale.y 1.6` を掛けると
   全高 2.56m で、静止時の中心は床天面 + 1.28。1.0 で置いたら床にめり込んだ状態から
   始まり、押し出しで浮き上がりながら歩くという分かりにくい壊れ方をした。
   デモの敵 2 体が y=1.35 なのはこれが理由 (M65f では偶然合っていた)。
5. ★**Collider の radius もスケールと二重に掛かる**。投擲物は `scale = 2r` だけで決め、
   `radius` は既定 0.5 のまま触らない。`radius = r` と書くと実効半径が r*2r になり、
   「何にも当たらない粒」ができる。
6. ★**sin / cos を CRT から取らない**。`Physics\AeroSampling.cpp` が
   「std::cos/std::sin は CRT 実装依存でビットが動きうる」と明記している。視点角は
   ハッシュ対象フィールド → 移動速度 → 足音の位置まで一直線に流れるので、9 次の
   多項式 (乗算と加算だけ) を自前で持った。`sin(x)=sin(pi-x)` の折り返し 1 段で
   |x| <= 3pi/2 まで受かる。**sqrt は例外** — IEEE-754 で正しく丸められるので使ってよい。
7. ★**視点は箱の 0.95 上では足りない**。天面が画面下端に帯として映り込む
   (probe のスクショで発見)。縦画角の半分 30 度に対し、0.275m 横の天面が 0.15m 下では
   29 度にしかならないため。1.10 なら 42 度で画角の外へ落ちる。**この種のずれは
   数式で詰めるより 1 枚撮るほうが速い**。
8. ★**カメラは既定で触らない**。`--acoustic-demo` の golden 2 枚は俯瞰なので、
   「マウスが窓の上を通ったら一人称」にすると**撮影中に手がマウスに触れただけで
   スクショ回帰が赤くなる**。切り替えは V キーのトグル 1 本だけ = 事故で入らない入力に
   限定し、カメラは `FindByName("Main Camera")` で掴んで LocalTransform を書く
   (描画レーンなのでハッシュには 1 ビットも出ない)。
9. ★**replay 7 ペア目の記録側に `--synth-input` を足した**。視点角は生マウスデルタの
   積分なので、無入力だと恒常ゼロのまま「配線ミスが記録側と検証側で対称に起きて
   一致してしまう」= mp ペアと同じ穴が開く。**M64a の ABI (GetMouseDelta) に実走の
   被覆が付いたのはここが初めて**。検証側には渡さない (mp と同じ理由)。
10. **企画 3-2 (速度が波の大きさになる) は歩幅でしか表していない**。音の大きさは床材が
    決める (M65c) ので、走ると「同じ大きさの波がより短い間隔で出る」形になる。
    振幅まで速度で変えるには `AcousticEmitter` に係数フィールドが要る = エンジンの変更
    なので「エンジンには 1 行も足さない」という M65g の狙いの外に置いた。
11. **F / Q / E は合成入力が押さない**ので、道具そのものは replay の被覆外。
    実装の確認は一時 probe (F 押しっぱなし + tick 30/250 で投擲) で行った:
    設置 150 tick で強度 0→2.19、回収 300 tick で 2.19→0.01、石は 11 m/s の放物線で
    壁に当たって age 63 で着弾、瓶は age 36 で着弾して割れる (破棄は tick 末なので
    その tick の波はちゃんと立つ)。捕まる経路は catchRadius を一時的に 40m にして
    carried 3→2→1→0 を確認した。
12. **投擲の着弾音はスクリプトが自分で鳴らす**。M65c の衝撃音は接触面の PhysMat から
    出るので、床材タイルの外 (素の床・壁) に落ちた石は 1 度も鳴らない = 企画 §5 の
    「投げた先の地形が一瞬見える」が成立しない。速度が落ちた tick に `pendingLoudness`
    を書く形にしてある (接触は見ない — `GetContactInfo` は Update からは常に 0)。
13. **敵と光は名前 (`FindByName`) で引いている**。デモ専用の割り切りで、ゲームとして
    作るならタグ検索が要る。1 度だけ引いて掴んだ ID をフィールドに持つ。
