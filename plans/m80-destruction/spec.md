...
| (2026-09-26 追記) ユーザー回答: 推奨を確定したもの | Notion 回答 (台帳「ユーザー判断」表に原文)。「これでいい」: 断面の描き方 / 上限の範囲 / 荷重モデル / `.mfrac` の置き場 / 破壊物の単位 / ボクセル化の見た目 | ユーザー確定 | 裁定どおり確定 |
| (2026-09-26 追記) ユーザー回答: 推奨を確定し「拡張性を意識して」と付いたもの | 同上。スキンの割れた後の描き方 / 固定壁の表し方 / 凹んだ破片の当たり / ABI の範囲 / libtess2 の取り込み | ユーザー確定 + 条件 | 裁定どおり確定。将来の拡張 (破片ごとのスキン描画 / 接地アンカー / 凸分解 / 専用の読み出し API / 三角形分割の差し替え) がどこに入るかを確かめ、足りなければ挙動を変えない構造変更で埋める (**sub-15**) |
| (2026-09-26 追記) ユーザー回答: 位相的な閉じ (T 字接合の解消) | 回答「実装は延期し再度検討/走査しそれでも無理だったら今後拡張する可能性があるから拡張性を意識して設計して」 | ユーザー判断 | 今は実装しない。前提が変わった状態 (libtess2・surface nets・凸包の修正後) で実現できるかを調査し、3 択の結論 (次で実装できる / 条件付き / 割に合わない) を ADR-021 に書く (**sub-15**)。調査で閉じる (コードは書かない) |
| (2026-09-26 追記) 打ち抜き・shot_verify の golden 4 枚 | 回答なし | 未回答 | 推奨の仮決定のまま (打ち抜きは後回し、golden は M80 で更新しない) |

| (review-1 #2) `.mfrac` の保存名が衝突する / 上書き後にライブラリが古いまま | 保存名 `<エンティティ名>_<seed>_<数>` は名前が同じ別物で衝突し、別の物の破片が黙って組まれた (review-1 P2c)。`FractureLibrary::LoadFromFile` は登録名が既にあれば読み直さない (`FractureLibrary.cpp:80-86`) | 裁定 | **保存名を内容から決める**: `<エンティティ名>_<16hex>.mfrac`。16hex は「焼きの入力 (ソースメッシュの頂点と index のバイト列、seed、pieceCount、openMeshMode、voxelResolution、焼き方式の版 `kFractureBakeVersion`) のハッシュ」。入力が同じなら同じファイルを共有してよい (中身がビット同一なので安全)。入力が違えば必ず別のファイルになる。加えて、書いたファイルが既に読み込み済みなら**読み直して登録し直す** (焼き方式の版を上げ忘れた場合の保険)。古いファイルは自動で消さない (AGENTS §1 の削除承認)。却下案: 連番 / エンティティの fileId = 同じ中身が重複する、シーンをまたぐと衝突する |
| (review-1 #4) スキンの破片の位置の意味 | スキンの破片は原点 = 骨の原点で、同じ骨の破片がすべて同じ位置になる。損傷の半径判定と onBreak の point が意味を失う | 裁定 | 破片の「位置」を**破片の体積重心**で定義し直す: 読み込み時に、破片メッシュ (外側面 + 蓋) の破片ローカルでの体積重心 `localCenter` を計算して持つ (`.mfrac` の形式は変えない。ファイルの中身の純関数なので決定的)。ワールドの位置は `破片のワールド行列 × localCenter`。スキンでない破片は `localCenter ≈ 0` |
| (review-1 #5) `strength` の既定値 5000 N | 20 kg・30 m/s の球でも、1 kg の箱を 5 m から落としても割れない。デモ・テスト・ベンチはすべて 100〜200 に下げている | 裁定 | 既定値を実測で決める。基準は 3 つ: 既定の Destructible (質量 1 kg・16 破片の箱) が 3 m から床へ落ちると割れる / 床に 600 tick 置いても割れない / 1 m から落としても割れない。この 3 つを同時に満たす範囲の中央 (対数で) を既定値にする。範囲が無ければ、その事実を返す (荷重モデルの見直しになる) |
| (review-1 #6) `voxelResolution` の範囲 16..256 | 開いた箱で、72 は成功、80 / 96 / 128 は数十秒後に失敗。取り消しが無く、終了時に焼き終わるまで待たされる | 裁定 | 失敗の理由を突き止める (上限や数値の限界なら記録。不具合なら直す)。そのうえで Inspector の範囲の上限を「開いた箱・16 破片で成功する最大」に下げる。焼きに**取り消し**を入れる (段階と切断の合間で旗を見る。Shutdown も取り消してから join) |
| (review-1 #7) 受け入れ条件 12 の絵 | 固定の壁は、どの frame でも無傷に見える。外れた内側の破片は、周りの破片に挟まれて動かない (打ち抜きが無いため) | 裁定 + `[聞ける #14]` | デモの配置を直し、壁を床から浮かせて**下寄り**を撃ち、外れた塊が重力で落ちて穴が見えるようにする (sub-18 で上の縁から変更: 上寄りの破片は真下の壁に支えられて落ちない)。受け入れ条件 12 の「抜けて見える」は、この配置で満たす。打ち抜きが無いことの見た目は、画像付きでユーザーに上げる (#14) |
...
- **打ち抜き** (割れた tick の衝突を、分かれた後の質量で解き直す)。v1 は、接着の判定が tick 末で、その tick の衝突は割れる前の塊 (kinematic なら無限質量) に対して解かれる。そのため、固定された壁に当たった弾はその tick で止まる / 跳ね返る。分かれた破片は、元の塊の速度 (固定の壁なら 0) から始まる。動的な塊では、塊全体が運動量を受けてから分かれる
...
4. 成分が 2 つ以上になった塊を分ける: ルートの塊 → 体積最大の成分がルートに残る。リーダーの塊 → リーダーを含む成分が残る。分かれた成分ごとに index 最小の破片をリーダーにし、`Rigidbody` を足してルートの親の下へ (ワールド姿勢を保つ `LocalTransform`)、残りのメンバーはリーダーの子へ。すべて tick 末のコマンドバッファ。**付け替えた破片の新しい `LocalTransform` も、tick 末の構造変更 (`ApplyStructuralChanges`) の直後に書く** (TickRunner が FractureSystem の後処理を明示的に呼ぶ。ハッシュを取る前)。これで同じ tick の中は「古い親 + 古い LocalTransform」、tick 末からは「新しい親 + 新しい LocalTransform」になり、階層と姿勢が食い違う窓ができない (review-1 #11、sub-17 round 1)
...
| 欄 | 型 | 既定 | 意味 |
|---|---|---|---|
| `fractureAsset` | AssetRef | null | `.mfrac` (他のファイル資産の AssetRef と同じ表現。review-1 #1) |
| `pieceCount` | Int32 | 16 | 破片数の目安 (2..256) |
| `seed` | UInt32 | 1 | 分割の seed |
| `openMeshMode` | Int32 | 0 | 0 = 拒否 / 1 = ボクセル化を許容 |
| `voxelResolution` | Int32 | 32 (sub-04 / sub-14 の計測。48 は開いた箱・16 破片で Release 10.8 秒) | ボクセル化の最長辺セル数 (Inspector は 16..72。80 以上は断面の閉じ判定で失敗する既知の限界、bench.md §10) |
| `innerMaterial` | AssetRef | null | 断面のマテリアル (null = ルートのもの) |
| `strength` | Float | 70 (sub-18 の実測。bench.md §9) | 接着の基準強度 [N] |
...
- 保存先: プロジェクト assets の `Fracture/<エンティティ名 (ファイル名に使えない文字を置換)>_<焼きの入力と kFractureBakeVersion のハッシュ 16hex>.mfrac` (review-1 #2 の裁定、§2。入力が同じなら同じファイルを共有する。書いた資産が読み込み済みなら読み直す。Undo は子エンティティ側だけ。古いファイルは自動で消さない)
...
21. (review-1) 焼いた `.mfrac` がセッションをまたいで解決される: 新しい AssetDatabase と FractureLibrary でシーンを開き直すと、最初の物理 tick より前に登録済みで、割れる。開き直した直後の Inspector に誤った「一致していません」が出ない — `--selftest` (セッションをまたぐ検査)
22. (review-1) 名前が同じ 2 つのエンティティを焼くと、中身が違えば別の `.mfrac`、同じなら同じ `.mfrac` を指す。焼き直しで中身が変わった資産は、ライブラリが読み直す — `--selftest`
23. (review-1) 縮んで消える挙動は、ルートの元の scale (非一様を含む) から比例で縮む。スキンの破片の損傷の半径判定と onBreak の point は、破片の体積重心で測る。割れた tick の中でも、破片の LocalTransform と階層が食い違わない。隣接の切り捨ては両側で対称 — `--selftest`
...
| sub-15 | 拡張点の確認と整理、位相的な閉じの再調査 (ユーザー回答の反映) | sub-12 | 20 (+ sub-15.md の条件) | M80m: 破壊の拡張点を整理し、位相的な閉じを再調査する |
| sub-16 | review-1: 資産の経路 (セッションをまたぐ解決・保存名・読み直し・資産キャッシュのキー・読み込みの検査・Inspector の一致表示) | sub-15 | 21, 22, 10, 20 | M80n: 破片資産をセッションをまたいで解決し、保存名を内容から決める |
| sub-17 | review-1: 実行時の正しさ (縮む挙動の scale、スキンの破片の位置、割れた tick の階層、隣接の対称な切り捨て) | sub-16 | 23, 10, 11, 20 | M80o: 破壊の実行時の不整合を直す |
| sub-18 | review-1: 調整と操作性 (strength の既定値、ボクセル解像度の上限と取り消し、デモの壁の配置、Inspector のウェイト照会のキャッシュ) | sub-17 | 24, 25, 26, 10, 20 | M80p: 強度の既定値と焼きの取り消し、デモの壁の見え方 |
...
13. 固定された壁を撃ったときの見た目 (弾が壁を打ち抜いて、破片が弾の方向へ飛ぶ UE 風の見た目にするか) / 裁定: v1 は打ち抜かない (割れた tick の衝突は割れる前の塊に対して解かれ、弾はその tick で止まり、破片は静止から落ちる)。打ち抜きは後回し / 逆 (M80 で打ち抜きを入れる) を選ぶと、割れた tick に物理をもう一度回して、分かれた後の質量で衝突を解き直すサブが 1 本増える (TickRunner と物理の段取りの変更で、replay 一致の検証も要る)。「分かれた塊に、受けたインパルスの一定割合を速度として与える」簡易案は運動量が保存しない不物理な調整値になるので、推奨しない
14. (review-1 #7) 打ち抜きが無いことの見た目を、画像で確かめてほしい (reviewer の画像: scratchpad の frac_8 / 35 / 60 / 200 / 590.png) / 裁定: v1 は打ち抜かず、デモの配置 (壁の上の縁を撃ち、外れた塊が重力で落ちる) で穴を見せる / 逆 (打ち抜きを M80 で入れる) を選ぶと、割れた tick の物理を分かれた後の質量で解き直すサブが 1 本増える (#13 と同じ論点。Notion の Q-14 に画像を添える)
15. (sub-18) ボクセル化の解像度の上限を 72 で止める裁定 (80 以上は、断面の閉じ判定が多頂点の境界ループで誤って失敗する。bench.md §10) / 逆 (M80 の中で閉じ判定を直して上限を上げる) を選ぶと、単一平面の切断の検算を作り直すサブが 1 本増え、焼きの digest の再ベースラインと決定性の再検証が要る。高ポリの実メッシュでも同じ経路を通るので、将来、実データで失敗が出たら直す前提で後回しにする
...
- ボクセル解像度 80 以上で、単一平面の切断の閉じ判定 (`VerifyCapOrientation` / `GeometricCapClosureValid`) が、多頂点の境界ループで誤って失敗する (bench.md §10、欠けの比 0.004〜0.03 で解像度に単調でない)。Inspector の上限を 72 にして避けた。高ポリの実メッシュでも同じ経路を通るので、実データで失敗が出たら最初に疑う箇所 (M80 の外の課題として ADR-021 の後回しに記録する)
...
- 2026-09-25 (sub-08 VERDICT round 2): §3 後回しに「打ち抜き」を追加し、v1 の挙動を明記。[ユーザーに聞ける] #13 を追加。round 1 で planner が不具合と判定した「壁の破片が数千 m/s」は、coder の机上計算の誤りで、実測では再現しなかった (分離の初速は 0、壁の破片の最大 2.9 m/s)。回帰の検査 (分かれた破片の最大の速さ ≤ 球の速さ × 2 × 1.05) は残す
- 2026-09-25 (sub-09 VERDICT round 1): 保存先の「ソース名」をエンティティ名に確定 (メッシュ登録名は `guid://...` で人が読めないため)。Reflection の欄のツールチップ (`MYE_FIELD_TIP`) は、既存の 143 か所と同じく英語のみとする。起動時に登録され言語切替に追随しない仕組みのため。§4.4 の「両言語」は `Tr()` を通す Inspector の文字列 (DrawComponentNotes 側) に適用する。欄のツールチップの多言語化はエンジン全体の課題で、M80 の外
- 2026-09-25 (sub-10 VERDICT round 1): スキンの骨の割り当ては、ボクセル化した入力では「破片の外側面の各頂点に最も近い元の頂点のウェイトを合計」とする (ボクセル化の頂点は元の頂点と位置が一致しないため)。骨空間への法線の変換は inverseBind の逆転置
- 2026-09-26 (sub-12 VERDICT round 1、Notion のユーザー回答): §2 に回答の反映行を追加。拡張性の確認と位相的な閉じの再調査を**新サブ sub-15** に切り出した (sub-12 は ABI と文書の締めとして完結しており、混ぜるとコミットの意味が閉じないため)。回答の番号は Notion 側の Q-n で、spec §7 の #n とは一致しない (対応は台帳)
- 2026-09-26 (sub-15 VERDICT round 1): ユーザー回答の反映を完了。拡張点 5 件の差し込み口と、位相的な閉じの再調査の結論 ((ii) 条件付きで可能、実行中の再分割 (B) に着手するとき専用マイルストーン) を ADR-021 に記録。全サブ (sub-01〜15) が OK になった
- 2026-09-26 (REVIEW_RESPONSE review-1): 指摘 13 件を受けた。planner 宛の 5 件 (#2 #4 #5 #6 #7) は仕様の穴として認め、§2 に裁定を追加。受け入れ条件 21〜26 を追加し、§4.2 の `fractureAsset` と `strength` の注記を更新した。coder 宛の 8 件と合わせて、修正サブ sub-16 / 17 / 18 に振り分けた (#1 #2 #8 #9 #12 → sub-16 (#9 は FractureSystem 側だが資産の解決の一部なので 16)、#3 #4 #11 #13 → sub-17、#5 #6 #7 #10 → sub-18)。既存サブの差し戻しにしなかった理由: 対象のコミットは全部入っており、指摘は複数のサブにまたがるため、テーマごとの修正サブのほうがコミットの意味が閉じる。[ユーザーに聞ける] #14 を追加
- 2026-09-26 (sub-16 VERDICT round 1): §4.3 の保存先の本文を、§2 の裁定 (内容から決める名前) に合わせて書き換えた (coder が指摘した矛盾)。同じ ID の中身が変わったときの RT の BVH / MeshColliderLibrary の無効化は入れない: 名前が内容から決まるので、同じ ID の中身が変わるのは `kFractureBakeVersion` の上げ忘れのときだけで、実害の経路が無い
- 2026-09-26 (sub-17 VERDICT round 1): §4.1 破断 4 に、付け替えの LocalTransform を tick 末の構造変更の直後に書くことを追加。round 1 の実装は、グローバルな注入 (`fracturesys::Install` / `Instance`) と ECS 外の対応表 (`reparentOverride_`) で `ApplyFractureDamage` の経路だけを補正していた。これは (1) 注入の有無でシミュレーションの結果が変わる (AGENTS §3.2 の暗黙的な登録)、(2) 対応表が tick の境界をまたいで残る (Update の冒頭でしか消えない)、(3) 汎用の位置クエリには窓が残る、ため採らない
- 2026-09-26 (sub-18 VERDICT round 1): `strength` の既定値を 70 N に確定 (3 基準の境界: 置いたまま割れない ≥ 1.8 N / 1 m 落下で割れない ≥ 53.5 N / 3 m 落下で割れる ≤ 91.7 N、対数中央)。3 つの境界は、荷重モデル (tick の接触インパルス / dt を、床に触れる約 5 破片で分け合う) からの見積もりと合う (1 kg の重さ 9.8 N ÷ 5 ≈ 2 N、1 m 落下 ≈ 265 N ÷ 5 ≈ 53 N、3 m 落下 ≈ 460 N ÷ 5 ≈ 92 N)。`voxelResolution` の Inspector 上限を 72 に。デモの壁は下寄りを撃つ配置に。ボクセル解像度 80 以上の閉じ判定の誤りを §7 リスクに記録
