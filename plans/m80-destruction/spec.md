...
| (review-1 #7) 受け入れ条件 12 の絵 | 固定の壁は、どの frame でも無傷に見える。外れた内側の破片は、周りの破片に挟まれて動かない (打ち抜きが無いため) | 裁定 + `[聞ける #14]` | デモの配置を直し、壁の上の縁を撃って、外れた塊が重力で落ちて穴が見えるようにする。受け入れ条件 12 の「抜けて見える」は、この配置で満たす。打ち抜きが無いことの見た目は、画像付きでユーザーに上げる (#14) |
...
| `strength` | Float | 5000 (仮。sub-18 の実測で決める) | 接着の基準強度 [N] |
