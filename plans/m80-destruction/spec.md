...
1. 位置のビット一致で溶接し閉じているか判定。閉じていて符号付き体積が負 (全面が内向きの巻き) なら**拒否せず全三角形を裏返して**外向きにそろえる (分割は外向きの入力を前提にする)。閉じていなければ `openMeshMode == 0` で拒否 (理由: 境界辺 n 本 / 非多様体辺 n 本 / 向き不一致 n 本)、`== 1` でボクセル化 + surface nets の閉じたメッシュへ置き換える
2. `Pcg32(seed)` で AABB 内の点を棄却法でメッシュ内部に `pieceCount` 個とる (内外判定はパリティ。試行回数に上限、足りなければ取れた数で進む)
3. 各シードの Voronoi セルを凸多面体として作り (AABB を膨らませた箱を二等分面で切る)、**セルの面だけで**メッシュを順に切る。切るたびに断面の閉ループを集め (穴を含む入れ子)、平面上で三角形分割して蓋にする。蓋の UV は**断面平面への正射影** (平面の正規直交基底 t,b への射影、1 m = UV 1。平面上なので歪みなし)、法線は平面法線
4. 切った結果を連結成分に分け、別成分は別の破片にする
5. 体積が平均の `minVolumeRatio` (既定 0.1) 未満の破片は、接触面積が最大の隣へ統合する (同値は index 小)
...
| `voxelResolution` | Int32 | 64 | ボクセル化の最長辺セル数 (16..256) |
...
3. Voronoi 分割が、箱と凹形状 (L 字、トーラス) で破片がすべて閉じ、体積和が元と相対 1e-4 以内、非連結が分離され、極小片が統合され、隣接 ≤ 32 — `--selftest`
...
| sub-06 | コンポーネント・破片エンティティ構築・root proxy 描画・`--fracture-demo` (壊れる前) | sub-03, sub-05 | 10, 11 (壊れる前), 16, 20 | M80f: Destructible / FracturePiece と破片の事前生成 |
...
| sub-09 | エディタ: Inspector・非同期焼き・生成の Undo・ローカライズ | sub-06, sub-04 | 14, 20 | M80i: 破壊物のインスペクターと非同期の破片生成 |
...
並列にできるもの: sub-05 は sub-01〜04 と独立。sub-04 は sub-02 / sub-03 と並列可 (sub-01 の閉じ判定だけに依存)。
