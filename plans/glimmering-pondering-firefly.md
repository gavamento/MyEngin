...
| M50c 構造上書き | **完了** | (このコミット) | 設計は敵対的検証どおりに成立。変異テスト = 復活抑止ゲート除去で selftest 8 本 FAIL を実証して復元。計画外 2 点: ①`MakeEditDocument` は**意図的に v2 のまま** (ミニシーンは refresh 対象外だが、将来呼ばれても「実体不変」に落とす — 旧アセットが開いて保存しただけで変わるのを防ぐ)。②新 API は計画の 2 本に加え `RemovedPrefabComponents` を追加 (Inspector の Restore 節は Engine 内部の ResolveBase を引けないため一覧 API が要る。実体・ベースと突き合わせて no-op 行を出さない)。**UI 3 点 (+バッジ描画 / Restore 節 / Revert Added メニュー) の実描画だけ手動確認待ち** (ロジックは selftest 36 check 被覆済み。バッジは全幅ヘッダに SameLine が効かないため draw list 直描き) |
| M50d codegen + ABI v11 | **完了** | (このコミット) | 計画外 3 点: ①「demo シーンに Health が既に載っている」は**誤り** (登録のみ・未装着) → DemoContent で Health + probe (SchemaHealthDemo) を**対ガードで装着** (単独では被覆にならない)。②MyeFieldType の追加は計画の列挙順でなく **Reflection.h の数値順** (Float4x4=13, String256=14 — M34 末尾追加の歴史をミラー。列挙順どおりだと型番号がズレて静かに壊れる)。③C# 糖衣は「MyeScript の protected」だけでは足りず **MyeEntity の public メソッド** (TryGetField 等) — 生成 .cs は別アセンブリで internal Engine にも protected にも触れないため。codegen 入力は実行時レジストリ (JSON 再パースなし = レイアウト二重実装なし)、生成ヘッダは static_assert 込みで selftest が %TEMP%\mye_selftest_schema_gen.h へ温存ダンプ → cl /W4 /WX 単体コンパイルで実証済み。文字列 Set は size<=cap + エンジン側尾部ゼロ埋め (String64 罠を境界で断つ)。NoHash (C#) は Get/Set とも遮断。assets/scripts/Generated/ は gitignore。検証: 変異テスト (Set 減衰 0.25→0.2501 → tick 0 MISMATCH 実証→復元) / C# temp probe (C++ probe の減衰を C# が読めた = クロスレーン整合、SetFaction も実走) / ミラー機械照合 71→73 スロット一致 |
