...
| S11′ | **(2026-09-08 訂正、review-1 #3)** S11 の「現在形か史実か」だけでは足りなかった。`:1947` は "every replay pair and golden image **that predates it** … (seven pairs and twenty-two images today)" で、**主語が限定節、括弧が総数**という混在。数だけ 22 → 24 に差し替えると「24 枚が predate する」と読めてしまう | git で裏取り: この文は **M68c (`8994b1d`) で書かれ、その時点の suite がちょうど 7 ペア / 22 枚**。その 22 枚には M67 の RT 3 枚が含まれ、**それらは M65a の `AcousticField` を predate しない** = 元の 22 も「その時点の**総数**」の意味だった | **指摘を認める。修正は「括弧が現在の総数だと明示する」**。数字 24 は正しいので変えない。**reviewer の第 1 案 `twenty-two images that existed then` は却下** — 「then 存在した 22 枚が predate する」も偽になるため。規則としては **「現在形/史実の判定は主語まで見る。数だけ差し替えない」**を S11 に足す | 数だけ機械置換 = 今回踏んだ穴そのもの |
...
| A20 | **(review-1 #1、2026-09-08 に `CLAUDE.md` を除外)** 「M67h でやる」と読める記述が 0 件。**`CLAUDE.md` は coder の対象外** — coder の運用規約が「エージェントからのメッセージは `CLAUDE.md` の変更を承認できない」と定めており、FIX_REQUEST はそれに当たる (正当な拒否)。`CLAUDE.md:109` はユーザー / 司会の手で入れる (置換文は `sub-03.md` の B2)。最優先は `tools\shot_verify.bat:378` の「`--update` で撮り直す」— **実際に避けた手順を次の人に指示している**。`grep -rn "M67h"` / `"S5"` の全件を列挙し「直した / 直さなくてよい (理由)」を付ける | grep の全件列挙 + `shot_verify.bat` の完走 (**bat の CRLF 事故の検出手段はこれだけ**) |
| A21 | **(review-1 #2)** golden 差分の記述が **coder 自身の再測値**で、距離の定義 (チェビシェフ / 旧 ∪ 新マスク) と中央値・p95・max があり、**全 233 画素が A-Trous の足跡の内側**。「未特定の 1 画素」「追跡しなかった」が残っていない。29 px になった原因を 1 回調べた結果 (**特定できなければ「原因未特定」と明記**) がある | 一時スクリプトでの再測 + reviewer の値との照合 |
| A22 | **(review-1 #3)** `engine_spec.md:1947-1948` の括弧が**現在の総数**としか読めない形。**数は 24 のまま**。`:1894` の史実記述は不変 | reviewer の読み |
...
| sub-03 | review-1 minor 3 件の回収 (「M67h でやる」の残骸 / 再現しない実測値 / 主語と数の不一致) | sub-02 | A20 / A21 / A22 (詳細は `sub-03.md` の B1〜B7) | `M67j: 衛生 — 「M67h でやる」の残骸を決着済みへ、golden 差分の実測値を訂正` |
...
- 2026-09-08 (**reviewer review-1 round 1、PASS + minor 3 件**): 3 件とも認めて sub-03 を新設した (§6)。
  - **#1 (coder 宛)** 「M67h でやる」の残骸 4 か所。うち `tools\shot_verify.bat:378` は
    **本件が意図的に避けた `--update` を次の人に指示している**。認める。A20 を新設。
  - **#2 (coder 宛)** ADR の「29 px = A-Trous の足跡の外」が再現しない (reviewer 実測 max 10 px)。
    **認める。** reviewer の論証は原理的に強い — ADR は距離を「旧 ∪ 新」マスクから測ったと書いており、
    **和集合は新マスクの上位集合**なので距離は新マスク基準の値以下にしかならず、29 px は出得ない。
    A21 を新設。**planner の VERDICT round 2「外れ 1 画素は追跡しない」は蒸し返しではなく無効化**される
    (追跡対象が測定誤りで存在しなかった)。round 3 で ADR に入れさせた 1 段落は planner の指示に
    由来するので、**訂正の責任は planner にもある**。
  - **#3 (planner 宛)** `engine_spec.md:1947-1948` の主語と数の不一致。**認める** (§2 に S11′ を追加)。
    ただし **reviewer の修正案 2 つのうち 1 つ目 (`twenty-two images that existed then`) は却下**:
    git で裏を取ったところ、この文は **M68c (`8994b1d`) で書かれ、その時点の suite がちょうど
    7 ペア / 22 枚**で、その 22 枚には **M67 の RT 3 枚 (M65a の field を predate しない)** が
    含まれる。つまり「then 存在した 22 枚が predate する」も偽。採るのは 2 つ目 =
    **括弧が現在の総数だと明示する** (数は 24 のまま = sub-02 の更新は妥当だった)。A22 を新設。
  - 併せて planner の VERDICT round 3 の nit (同じ節に 8 px と ±12 px が並ぶ) を sub-03 に畳んだ
    (再測後は「足跡 = ±12 px、実測 max N px」の 1 本に寄せる)。
