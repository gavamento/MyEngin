...
| M63d ライティング | **完了** | (このコミット) | **段階①は成功** — `LightSample` の抽出で golden 17 枚が maxDiff=0、複製ミラーへの縮退は不要だった。ただしビット保存の条件は「**影を LightSample の外に残す**」こと: 平行光は `atten = dirShadow` の**代入**、局所光は `atten *= localShadow[i]` の**乗算**という非対称が既存の演算列そのもので、`atten *= (type==0 ? dirShadow : localShadow[i])` の 1 本に畳むと平行光に `1.0f *` が 1 つ増える。計画外の事実 4 点: ①**`particle_light.hlsli` は register 宣言を持たせた** — 計画は「純関数の束」を想定していたが、CB 1264B とライト配列のレイアウトを 2 シェーダへ手写しする方が危険なので `terrain_common.hlsli` (M58d) の前例に倒した。スロットは CPU/GPU で違う (CPU `b1/t4/t5/s1` / GPU `b2/t5/t6/s1`) ので**マクロで渡し、既定値を置かず `#error` で止める** — 既定 t4 のまま GPU が include するとフロクセルと衝突して**エラーも出ずに**絵が壊れる。②**補間子の追加は `posW` 1 本だけ**。mode=1 は受光を `o.color.rgb` へ畳むので TEXCOORD が増えない。ただし**光を測る位置は四隅ではなく粒子の中心** (`p.pos` / `basePos`) — 四隅で測ると板の上にグラデーションが乗って「粒子単位」でなくなる (点光源に近いほど顕著)。③**VS ステージにもリソースを張る**。mode=1 は VS で `ParticleLightAt` を呼ぶので CB / CSM / irradiance / 比較サンプラ / s0 が VS 側に要る。1 本でも落とすと未バインド SRV の `SampleCmpLevelZero` が 0 を返して `dirShadow=0` = **そのエミッタだけ暗く沈む**。④**mode=1 は golden 被覆がゼロだった** (デモの P4_Lit は mode=2 だけ) — ユーザー決定で 6 本目 `P4b_LitVertex` (x=-16) を追加した。ランプも影の箱も**相対位置までミラー**し `lightingMode` 以外は 1 フィールドも同じ = 2 モードの純粋な A/B。既存 5 本の位置は 1 つも動かしていない (動かすと C1〜C3/C5 の被覆が「位置が変わっただけ」で全部赤くなる)。**実測: 既存 15 枚は maxDiff=0 で不変 / ライティング領域は CPU/GPU で 0 画素差 (共有 hlsli が効いている証拠) / 飽和画素 0・最大 227→251 = 白飛びさせず回帰検出力を保っている / CPU-GPU 全体差 158 画素は M63a の回転領域と M42e の衝突領域だけ**。★申し送り 2 点: (a) `lightReceiveShadow` (エミッタの意思) と `shadowBound` (CSM を張れるか) は**別物**で、両方を掛けてから CB へ入れてある — シェーダ側で二重に判定しないこと。(b) godray は screen-space なので、粒子が明るくなるとシャフトも強くなる (増幅であって二重計上ではない) |
| M63e 深度衝突 | **完了** | (このコミット) | 計画外の事実 4 点: ①**5 タップ法線はスクショ回帰では守れない** — 実測: 既定のデモには平らな床 1 枚しか衝突面が無く、±1 タップが必ず同じ面に載るので **2 タップへ戻す変異を入れても golden が maxDiff=0 のまま通った**。シルエットを作る箱を落下域へ置いてみると 1.3x0.9x1.3 の段差では 43 画素動くのに、ひと回り小さい 0.9x0.6x1.3 では **0 画素**に戻る — frame 120 のその瞬間に 際の 1 画素へ粒子が居るかどうかの運で決まる = 回帰検出の道具にならない。デモ追加は撤回し、選択規則を `ParticlePickPlusTap` へ切り出して selftest 5 本で機械照合する形にした (`ParticleClipToUv` と同じ扱い)。②**デモへ物を足すなら必ず関数の末尾**。`CpuParticleBackend.cpp:229` が `pool.rng.Seed(desc->seed, e.index*2+1)` = PCG の**ストリームがエンティティ番号そのもの**なので、エミッタより前に箱を 1 個足しただけで **CPU の絵が 31871 画素動いた** (CPU は衝突しないのに、である)。M63d の「既存 5 本の位置を動かさない」と同じ制約が **生成順**にも掛かっている。③摩擦の分解は接線側にも restitution を掛ける — 既存の純反射 `(v-2vn)*r` が `r*vt - r*vn` と**接線にも反発を掛けていた**ので、「接線は (1-friction) だけ」にすると friction=0 が既存と一致しない。その上でなお早期 return が要る (代数は一致しても演算列が違う)。④`collParams.w` の予約枠を摩擦へ回し、反射の 4 引数 (enabled/restitution/thickness/friction) を 1 レジスタに揃えた — 新設した `collParams2` は「反射しない側」(寿命損失 / 解析床) だけ。**実測: 既存 16 枚 (particle_cpu 込み) は maxDiff=0 で不変**、particle_gpu だけ 334 画素 / x[663..717] y[271..286] = P5 の衝突領域に完全に閉じている。CPU-GPU 差は 158→310 画素で、内訳は M63a の回転領域 4 画素 (不変) と衝突領域 306 画素のみ。★申し送り: 解析床は深度衝突と**独立ゲート**なので、両方 ON の tick は寿命損失が 2 回引かれる (床の手前に面が映っているなら 2 回ぶつかったのは事実 = 意図した挙動) |
