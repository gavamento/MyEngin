...
## 申し送り (セッション跨ぎ)
- (sub-07 → ユーザー / S5 の手順、spec §4.6 の「ユーザー」行) (1) `cmd /c bind\Release\Editor.exe --render-demo --deferred --rt-refl --rt-restir` (と `--acoustic-demo` の同構成)。エディタは乱数を凍結しないので動くノイズがそのまま見える。(2) 表示 → RT Debug → ReSTIR サブメニューで実行中に触る (クラス表 5 行 × 半径/タップ/M 上限、α 基準、Spatial reuse、Visibility ray、クラス上書き、SVGF 履歴、A-Trous、Reset)。非永続。(3) 見るもの: `--rt-debug 12` (M ヒートマップ) / `13` (一次ヒットのクラス) / `14` (反射像側)。(4) **最初に触るノブは M 上限** — 半径・タップは spatial が既定 off の間は効かない
- (sub-07 → 後続 M67h) 焼く項目: `RtTypes.h` の `kRtReflClassTable` 5 行 / `kRtRestirRadiusAlphaRef` / `RtReflRestirParams` の `spatial` (on を選ぶなら 0 → 1) / `visRay` / `svgfHistory` / `atrousIterations`。firefly が出ていたら `kRtRestirWMax`。焼いたら `shot_verify.bat --update` で `demo_render_rtrefl_restir` を撮り直し、他 21 枚が動いていないことを `git status` で確認してからコミット。文書衛生: engine_spec §11.3 の歴史的記述 ("all fourteen other shots...")
- (sub-07 → 後続、nit) bat の frame 指定枠 (physics / joints / fog / particle / acoustic / ReSTIR) は SHOTBASE を展開せず直書き — SHOTBASE の解像度を変える日が来たらまとめて追従する
- (sub-07 → reviewer) 最終ビルドは `/p:MyeWarnAsError=true` で回して記録に残す (coder は立てずにビルド。コード変更はコメント 2 行のみ)。読む順: ADR-016 → engine_spec §6.4 の "ReSTIR reflections and `ReflectionClass`" → §11.3 の RT 3 枚の段落。ADR の各決定に対応する spec §2 の S 番号は S1 / S2 / S3 / S4 (U4) / S5 / S6 / S7 / S8 / S12 / S15。golden の実物は `tests\golden\demo_render_rtrefl_restir.png` (frame 40)、ReSTIR off の同フレームは `testsctual\m67g_f40_off.png` (gitignore、再撮影可)。受け入れ条件 5 (ADR ⇄ engine_spec の整合) は reviewer の読み判定
