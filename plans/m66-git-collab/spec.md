...
| S14 | `shot_verify.bat` の acoustic 2 枚 (golden 18/19) が M66 以前から不安定 | 同一 Release バイナリで 1/4〜1/2 の確率で座標 (237,443) 付近 ~40×40 画素が動く (sub-08 で実測。sim は replay でビット一致 = 描画側の run-to-run 非決定性)。golden は M65g、その後 M65h (`04b0733`) が `AcousticField` / `RenderSystem` / `TickRunner` に触れている。sub-08 の Engine 層差分は Particle 系のみ | **c. 何もしない** (2026-09-03) | M66 では `ci.yml` も CLAUDE.md も触らない。ローカルは再実行で通し、CI は赤くなったら再実行。根治は M66 の外 (反対意見 = a の CI 降格を推奨したが押し切られた。以後蒸し返さない) |
...
色は `themeColor::*` のみ。
**バッジの色表 (sub-09 round 1 で確定。正本は `EditorWidgets.cpp` の `ScmBadgeColor` 1 箇所、Source Control 窓と Content Browser が共用)**: M = `Warning` / A = `Success` / D = `Error` / R = `Prefab` / 競合 = `Error` (グリフ `!` で D と区別。競合中は一覧が競合モードに切り替わるので曖昧にならない) / ? = `TextDisabled`。**`Accent` 系を状態色に使わない** (`ImGuiTheme.h` 配色ルール 3。選択行の Accent の面に Accent のバッジが載ると読めない = `cache/m66i_zoom_tree.png` で実測)。セルフテストで全状態について `ScmBadgeColor(state) != themeColor::Accent` を固定する。文言は `LocalizationTable.inl` en/ja、`###SourceControl*` の ID 一意。
...
**幅の規則 (review-1 #3 と sub-12 round 1 の実測で確定)**: 既定ドックの左列は ≒ 287 px しかない。
ここに出す**文**は折り返す (`TextWrapped`。色が要るなら `PushStyleColor` + `TextWrapped` — `TextDisabled` は折り返さない)。
折り返さないのは**語**だけ — パス / ブランチ名 / 短い状態語 (`Scm_Loading` など) のように、
途中で切れても意味が壊れないもの、および横スクロールする子窓・ツリー行の中にあるもの。
**文言を短くして収める直し方は採らない** (説明を削るのは直しではない)。判定の基準は **ja** で撮った 287 px の画面
(同じ内容で日本語のほうが幅を食う)。**推測で全箇所を触らない** — 撮った画面に切れて写っているものを直す。
...
| 21 | 既定ドックの左列 (287 px) で Source Control 窓の**文**が途中で切れない (§4.3 の幅の規則)。ja で確認 | 実機スクショ (en / ja の 2 枚。切れて写っているものが無いこと) |
| 22 | `MYE_COLLAB_PROBE` の恒久検査が**標準の fixture でそのまま走る** (未追跡ファイルがあるだけで飛ばされない)。飛ばすのは「index に staged がある / マージ途中」= プローブがリポジトリを変えてしまう場合だけ。**サイドカーだけが status に載った行は index 側の内訳を持たない** (`PairedEntry::indexState` は本体の行にしか入らない) ので保守的に飛ばす — 過剰に飛ばす側は許容、走らせて commit が成功してしまう側は不可 | `MYE_COLLAB_PROBE=<fixture> Editor.exe --selftest` の PASS 行数を、未追跡を含む fixture で数える |
...
| sub-12 | M66l: review-1 の衛生 — ヒント文の折り返し / 腐ったコメントと未使用アクセサ / EndBatch のコメント / 既知の制約と検証フックの記載 | sub-11 | 15 (追記分), 21, 22 | `M66l: 衛生 — 折り返しとコメントの実態合わせ、§14 に既知の制約と MYE_COLLAB_PROBE` |
