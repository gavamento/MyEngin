...
> ## ★この計画は M70 へ移管した (2026-09-07)
>
> M69f が申し送った「ラベルを M64 のまま使うか新番号にするか」はユーザー判断で
> **新番号 M70** に決着した。実装の正本は **`plans\sparkling-gliding-quokka.md`
> (M70: 地雷除去)** で、本ファイルの M64a / M64b / M64c はそれぞれ
> **M70b / M70c / M70d** の材料として引き継がれている。
>
> - 本ファイルは**調査結果と判断 1〜5 の記録として読む**。サブのラベルと版番号は
>   M70 側が正本 (`.rep` v5→6 / snapshot v11→12 / netproto v3→4 / ABI v15→v16 = 110 スロット)。
> - **判断 1 (キャンバス) は M70 で差し替わった**。本ファイルは「1920x1080 固定 +
>   スケール」を前提にしているが、M70b は **Unity / UE 準拠の可変キャンバス**
>   (`s = min(w/1920, h/1080)`、`canvas = 実 px ÷ s`、レターボックス無し) を採る。
>   `canvasW/H` も `mouseCanvasX/Y` と一緒に `InputSnapshot` へ記録する。
> - M70 は 4 サブ (M70a シリアライザのラウンドトリップ / M70b キャンバス /
>   M70c UI イベント + ABI v16 / M70d 穴埋め) で、**M70a が本ファイルに無い新規**
>   (dogfooding #10 のデータ消失)。
