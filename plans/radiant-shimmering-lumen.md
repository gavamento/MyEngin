...
| Forward `t7` | froxel 積分結果 | M57e (透明後段 / スカイ / パーティクル。M57d は Deferred の不透明だけ = `IRenderPath::AppliesFroxel()` が Deferred のみ true) |
...
| 12 | `demo_render_froxel` | 0 | × (`MYE_SHOT_SKIP_FROXEL`) | **M57d** (M57c から移送。積分結果を読む者が居ない M57c で撮ると `demo_render_deferred` と tol=0 でビット一致する 2 枚目にしかならず、動いたときの切り分けができなくなる。実測済み) **→ M57d で撮影済み**。tol=0 の根拠 = 同一引数 2 回 / Debug↔Release とも maxDiff=0。CI へ載せない根拠 = TAA と同型の蓄積系で増幅率未測 + WARP 撮影が 12s 増える |
