...
## 統合後に見つかった別件 (M59 候補。このロードマップの範囲外)

### `net_verify.bat` の case D が約 5 回に 1 回落ちる — DLL シャドウコピーの残存レース

**症状**: `[net] host rejected the connection: starting world hash (different scene or build)` で
両プロセスが 0 tick で終了 (exit 1)。統合 2 の直後 1 回発生、その後 4 回連続で緑。
落ちたときの `crash` リングのスナップショットは **host 2969 バイト / joiner 3289 バイト** =
2 プロセスが**構造の違う世界**を組んでいる。

**機構** (コードで確認済み。推測ではない):

1. `DllReloader::IsWritable` (`src\Engine\Engine\HotReload\DllReloader.cpp:90`) は
   「リンカがまだ書いているか」を見るために `bin\x64\<Config>\GameLogic.dll` を
   **`dwShareMode = 0` の排他オープン**で開く。
2. `LoadInitial()` (:138) は `TryCopyAndLoad()` を **1 回だけ**呼ぶ。`IsWritable` が偽なら
   即 false を返し、`EngineLoop.cpp:283` が「NO C++ scripts are registered」を出して**続行する**。
3. ネット対戦は 2 プロセス**同時**起動が常態。両者が同じ 1 個の DLL を同じ瞬間に排他オープン
   すると片方が必ず負ける。
4. `Update()` の再試行は `kPollIntervalMs = 500` 待ちで、**開始ワールドハッシュの照合は
   ハンドシェイク時 = それより前**。よって「片方だけ C++ スクリプト 14 本を持つ世界」が
   確定したまま照合に入り、拒否される。

M52h はシャドウ棚を `p<pid>` へ分離して**コピー先**の衝突は消したが、**コピー元を読む
排他オープン**は共有のまま残っていた。

**直し方の案**: `LoadInitial()` の `IsWritable` を短い有界リトライにする
(起動時 1 回きりなので数十 ms のスピンで足りる)。`Update()` 側の 500ms ポーリングは
そのままでよい — 問題は「起動時の 1 発勝負」だけ。

**→ 修理済み** (greedy-cooking-wave.md の進捗表「(別件)」行のコミット参照)。採ったのは
リトライより一段強い形: プローブ (`ProbeWritable`) の share を 0 → `FILE_SHARE_READ` にし、
**読み手同士 (= 2 エンジンプロセス) の衝突を原理的に消した** (共有検査は双方向なので、
こちらが FILE_SHARE_WRITE を許さない限り書き手 = リンカは従来どおり弾ける)。保険として
`LoadInitial()` に共有違反 (32) 限定の有界待機 250ms も追加 (案の「有界リトライ」に相当)。
再発時の検出線として net_verify.bat が全ケースで「NO C++ scripts are registered」を
findstr するようにした (両プロセスが同時に 0 本 → ハッシュが偶然一致して緑、の穴も塞ぐ)。
共有セマンティクス自体は `DllReloaderSelfTest.cpp` が固定している。

**描画ロードマップとは無関係**: 失敗はハンドシェイク時点 = tick 0 より前で、
描画コードは 1 行も走っていない。ワールドハッシュは sim 状態だけを見ており、
M54〜M58 が足したコンポーネントは全部 `kComponentNoHash`
(唯一ハッシュ対象になった `LightComponent::castShadow` は両プロセスで同値)。
ただし**このロードマップが起動コストを増やした**ぶん、既存レースの窓が広がった
可能性は否定できない。
