...
#### 10.2.1 Sub-asset keys (M74a / M74b)

Meshes, materials, skinned models and embedded textures that come out of a model file are
registered under a name, and their AssetID is `HashStr(name)`. That AssetID is what scenes, prefabs
and actors store. See [ADR-019](docs/adr/ADR-019-guid-subasset-keys.md).

| Concept | Decision |
|---|---|
| Name format | `"guid://<16hex>#mesh3#part0"` (FBX), `#mesh%u#skin%u`, `#mat%u`, `#defaultmat`, `#tex:%u`; glTF uses `#mesh%zu#prim%zu`, `#mat%zd`, `#skin%zu`, `#img:…`. The 16hex is `assetkey::Resolve(NormalizePathKey(modelPath))` = the model's `.meta` GUID |
| Single producer | `assetkey::SubAssetKeyPrefix` (Core). Both loaders store it in `LoadContext::keyPrefix`; nothing else builds a prefix |
| Why not the path | Before M74a the prefix was the normalized **absolute** path. Two machines with different clone paths each saw only the models the other had not placed, with no log line (unresolved AssetIDs are skipped by the draw loop). A `.meta` GUID is committed with the model, so the ID is the same on every checkout and survives moving the model inside the project |
| Why not an assets-relative path | Moving a model would still change every ID, the loaders would need the root passed in, and files outside the root have no relative form |
| Fallback | A model without a `.meta`, or any load while no resolver is installed (`--selftest`), gets the legacy path-hash as its 16hex. Such IDs are checkout-dependent again, so `.meta` files for models must be committed |
| Reverse lookup | `ConvexCookSourcePath` parses the GUID with `assetkey::ParseSubAssetKey` and resolves the current path through `assetguid::ResolvePath`; unresolved means "build in place" |
| Migration | `Editor.exe --migrate-subasset-ids [--project DIR] [--legacy-root OLD]... [--dry-run]` scans and registers every model headlessly, rebuilds the legacy name for each current name under every legacy root (the current checkout is always included), and rewrites matching integers in `*.scene.json` / `*.prefab.json` / `*.actor.json` as text, so a diff touches only the ID digits. A legacy ID can only be recomputed if the absolute root that produced it is known, and a model that has since moved inside the project cannot be mapped |
| Versions | `kCookVersion` 3. The scene document, `.rep` and snapshot versions are unchanged: layouts did not change, and no committed replay or hash depends on these values |
| Tests | `SubAssetKeySelfTest`: the same model at two absolute paths registers identical names and IDs; without a `.meta` they diverge (control); convex source lookup; legacy recomputation for this and another checkout; text rewrite; end-to-end migration |
...
| Cooking | `.mcvx` sits beside `.mmdl` and `.mpcm` in `CookedCache`, keyed by walking `MeshLibrary`'s registered name back to the source path (`"guid://<16hex>#mesh0#prim0"` → `assetguid::ResolvePath`, §10.2.1). Procedural meshes have no `guid://` prefix, so no source path is found and they build at runtime with **no special-case code**. Sealed caches are never appended to |
...
- One repository open in two editors at once is unsupported (git's `index.lock` error is shown
  as-is), and paths beyond 260 characters are not supported. Clone paths no longer need to match:
...
| 10 | Texture cache / model format | **Cooked cache under `cache/cooked/`** | `.mmdl` / `.mpcm` blobs invalidated by size, mtime and content hash (§10.2), BCn DDS texture cook, glTF via cgltf and FBX via ufbx. A packaged build ships a **sealed** cache so it replays without re-parsing after relocation; sub-asset IDs themselves are relocation-proof since M74a (`.meta` GUID keys, §10.2.1) |
