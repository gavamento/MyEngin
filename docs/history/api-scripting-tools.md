...
- (v21 / M78e / 2026-09-22) Compute ABI 7 本 (CreateComputeBuffer / ReleaseComputeBuffer / SetComputeBuffer / SetComputeFloat / SetComputeFloat4 / SetComputeTextureFromAsset / DispatchCompute)。既存スロットは 1 本も動かしていない (末尾 append)。GPU バッファの確保・解放・dispatch を C ABI 経由でスクリプトへ開通した。生 D3D 型は Shared に出さず不透明 uint64_t ハンドルで運ぶ (メモリ解放はエンジン側 = ComputeAbiRunner::Shutdown。シーン遷移でも回収する)。名前ベースのバインドは ShaderProgram::csBytecode を D3DReflect で照合する。組み込みテクスチャは HashStr("white") と HashStr("builtin://white")。未知名・未解決 AssetID は 0 で落ちない。★C# レーンは record/verify 中に走らない (replay 被覆外)。Compute 結果を ECS / WorldHash へ書き戻す用法は禁止 (spec §4.1)。check_rules.ps1 の $apiVersionSlots に 21 = 125 を追加。C# 糖衣は Engine (internal) と MyeScript (protected) の両方。
- (v22 / M80l / 2026-09-26) 破壊 (M80) のスロット 1 本 ApplyFractureDamage (末尾 append) + `MyeScriptDesc` 末尾のイベント `onBreak` (null 可)。ApplyFractureDamage は sub-07 で先に実装済みだった Engine 層の内部関数 (FractureSystem.h) をそのまま呼ぶだけ。onBreak は FractureSystem が分離を確定した tick 末に、ルートにあるスクリプトへ (C++ は ScriptHost::DispatchBreak、C# は ManagedHost::DispatchBreak + InvokeBreak)、ルート index → 新リーダー index 昇順で配信する。配信順を保証するため、FractureSystem::UpdateImpl は Destructible の処理順を ForEachArchetype の行順ではなく root の EntityID.index 昇順へ並べ替えるようにした (単一 Destructible しか壊れない既存シーンでは観測できない差分)。check_rules.ps1 の $apiVersionSlots に 22 = 126 を追加。C# 糖衣は Engine.ApplyFractureDamage (internal) と MyeScript.ApplyFractureDamage / OnBreak の両方。
