...
| **16** | **`LoadGame` がセーブ時のシーンへ必ず遷移する** | **修正済み (M70c、ABI v16 の `LoadPersist`)** |
...
**→ 解決 (M70c)**。ABI v16 に `LoadPersist(slot)` を足した（C++ 側の糖衣は
`MyeLoadPersist`）。`PersistStore` だけを置換し、`pendingScene` には触らない。
消費点は `LoadGame` と同じ tick 末のセーフポイントで、record/verify/netplay 中は
no-op + WARN も同じ（セーブファイルは sim の外にあるので、再生を跨がせない）。
上の「タイトルでセーブを書く」回避策と `saveLoaded` の旗はもう要らない。
