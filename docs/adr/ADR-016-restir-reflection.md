...
**旧 + 新の Hero マスクから 8 px 以内に 232 / 233 (99.6%)** が収まる — 8 px は
半解像度 (480x270) の A-Trous (5x5 × 2 反復) + 双一次拡大の足跡そのもの。
副産物として `--rt-debug 14` の Hero 画素は 384 → 400 に増えた (Hero 候補が長く生き残るため)。
**「1 行変えたのに絵が動かない」ではない**ことをここで確認している。

★**残る 1 画素は未特定**: 233 画素のうち 1 つだけ最寄りの Hero 画素まで 29 px あり、
A-Trous の足跡 (全解像度で ±12 px) の外にある。候補が Hero ⇄ Default で入れ替わった 16 画素の
影響がデノイザ越しに届いた可能性が高いが**確定していない** (maxDiff ≤ 5 の 1 画素で、
原因がどちらでも既定値も golden も変わらないため追跡しなかった)。
