...
- **M72i (任意)**: 半透明の実メッシュのゴースト。第 2 の World を分岐スナップショットから復元して
  `viewKey=0` で描くには RenderSystem に「全ドローを半透明ティント」経路が要る。ワイヤ + トレイルで
  目的 (どこで何が違ったか) は満たせるので据え置き。
