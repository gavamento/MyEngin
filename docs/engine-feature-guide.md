...
現在の ABI は `MYE_API_VERSION = 17` です。v17 の `GetSceneName` は、環境依存の絶対パスをゲーム状態に持ち込まず、シーン名を取得するための入口です。
...
| Skybox | 色のグラデーション、DDS cubemap | Cubemap は実装済み。解決失敗時はフォールバック |
...
`project.mye.json` が名前、エンジン版、起動シーン、canonicalRoot を持ちます。`--project` で外部プロジェクトを開けます。プロジェクトローカルなエディタ設定は `.mye` に分離します。Project.h の表示用バージョンは `0.66` で、ABI v17 や開発マイルストーン番号とは別の値です。
...
| 項目 | 調査時の判定 |
|---|---|
| ABI v16 / 110 スロットという README 記述 | 現行コードは v17。公開関数一覧はリファレンス参照 |
