...
public:
    ...
    // cubemap または 2D パノラマ ソース (キー = AssetID)。初回はベイク、以後キャッシュ
    EnvMaps GetForCubemap(GraphicsDevice& device, ShaderManager& shaders, AssetID id,
                          ID3D11ShaderResourceView* src, bool isPanoramic = false);
    ...
    // M56e: 任意の cubemap / panoramic SRV から IBL 一式を焼く (**キャッシュを通さない**)。
    ...
    bool BakeFrom(GraphicsDevice& device, ShaderManager& shaders, ID3D11ShaderResourceView* src,
                  BakedEnv& out, bool isPanoramic = false);
...
private:
    ...
    bool Bake(GraphicsDevice& device, ShaderManager& shaders, ID3D11ShaderResourceView* src,
              const GradientColors& grad, BakedEnv& out, bool isPanoramic = false);
