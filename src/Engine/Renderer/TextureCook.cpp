...
bool CookImageToDds(const std::wstring& srcImagePath, const std::wstring& dstDdsPath)
{
    ...
    const uint32_t topLinear = static_cast<uint32_t>((w + 3) / 4) * ((h + 3) / 4) * blockBytes;

    DdsHeader hdr;
    std::memset(&hdr, 0, sizeof(hdr));
    hdr.size = 124;
    hdr.flags = kDdsdCaps | kDdsdHeight | kDdsdWidth | kDdsdPixelFormat | kDdsdMipMapCount
        | kDdsdLinearSize;
    hdr.height = static_cast<uint32_t>(h);
    hdr.width = static_cast<uint32_t>(w);
    hdr.pitchOrLinearSize = topLinear;
    hdr.mipMapCount = mipCount;
    hdr.ddspf.size = 32;
    hdr.ddspf.flags = kDdpfFourCC;
    hdr.ddspf.fourCC = hasAlpha ? MakeFourCC('D', 'X', 'T', '5') : MakeFourCC('D', 'X', 'T', '1');
    hdr.caps = kDdsCapsTexture | kDdsCapsMipMap | kDdsCapsComplex;
    ...
    MYE_LOG_INFO("[cook] %s -> %s (%dx%d, %s, %u mips, %zu KB)", srcUtf8.c_str(),
                 WideToUtf8(dstDdsPath).c_str(), w, h, hasAlpha ? "BC3" : "BC1", mipCount,
