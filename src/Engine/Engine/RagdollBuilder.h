...
struct Options {
    ...
    // ★M60j で `JointComponent.disableCollision` が入り、隣接ペアの接触は外せるように
    //   なった (下の `disableCollision`。**既定で立てる**)。ただし**この比は 1.0 に
    //   戻していない** — 実測すると除外を立てても入眠が tick 395 → 241 へ早まるだけで、
    //   **substeps 16 の要求は 1 段も下がらなかった** (2 / 4 / 8 は立てても立てなくても
    //   一生眠らない)。この rig が 16 を要求している原因は隣り合う骨の食い込みではなく
    //   「床に触れながら関節に吊られている」ほうで、幾何の逃げ (この比) と除外は
    //   **別の問題に効く別の道具**。両方効かせて 241 tick が現状の最良値。
    float lengthRatio = 0.8f;
    // 生成した Cone 関節で「親子の骨どうしの接触」を切る (M60j / M60k)。
    // ★**既定 true**。骨は関節点を共有しているので、隣り合うカプセルは姿勢によっては
    //   必ず食い込む — その接触は暴れの原因でしかなく、押し返しが関節と噛み合って
    //   入眠を遅らせる (実測 395 → 241 tick)。切るのは**直接繋がった 1 ペアだけ**で
    //   伝播しないので、腕と胴のような 1 つ飛ばしの当たりは残る (M60j の申し送り 2 の
    //   「自己衝突を丸ごと切ると却って暴れる」に触れない範囲)。
    // ★substeps の要求は下がらない (上の lengthRatio のコメント)。これを立てたから
    //   といって 16 を 8 へ落としてはいけない。
