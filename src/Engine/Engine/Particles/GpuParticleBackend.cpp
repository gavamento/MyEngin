...
struct GpuParticleBackend::EmitterTickCtx {
    ...
    float dt = 0.0f;
    bool localSpace = false;      // M61g: ローカルシミュレーション空間
    XMFLOAT3 origin = {};         // 放出原点 (ローカル空間なら 0)
    ParticleEmitBasis basis = {}; // M61b: CPU バックエンドと同じ基底
    XMFLOAT3 emitterVel = {};     // M61c: エミッタ速度 (速度継承)
    XMFLOAT3 prevOrigin = {};     // 今 tick の補間用 (上書き前に退避した値)
    uint32_t prevOriginValid = 0;
    ID3D11DeviceContext* dc = nullptr;
    ID3D11ComputeShader* emitShader = nullptr;
    ID3D11ComputeShader* simShader = nullptr;
};
...
void GpuParticleBackend::Update(World& world, float dt)
{
    ...
    for (GpuEmitter& em : emitters_) {
        ...
        // 1 tick ぶんの手順は RunEmitterTick。プリウォームがこれを誕生 tick に k 回先回しで呼ぶ
        EmitterTickCtx tick;
        tick.desc = desc;
        tick.em = &em;
        tick.dt = dt;
        tick.localSpace = localSpace;
        tick.origin = origin;
        tick.basis = basis;
        tick.emitterVel = emitterVel;
        tick.prevOrigin = prevOrigin;
        tick.prevOriginValid = prevOriginValid;
        tick.dc = dc;
        tick.emitShader = emitShader;
        tick.simShader = simShader;
...
bool GpuParticleBackend::RunEmitterTick(EmitterTickCtx& t, bool allowIdleSkip)
{
    ...
    GpuEmitter& em = *t.em;
    const float dt = t.dt;
    const bool localSpace = t.localSpace;
    const XMFLOAT3& origin = t.origin;
    const ParticleEmitBasis& basis = t.basis;
    const XMFLOAT3& emitterVel = t.emitterVel;
    const XMFLOAT3& prevOrigin = t.prevOrigin;
    const uint32_t prevOriginValid = t.prevOriginValid;
    ID3D11DeviceContext* dc = t.dc;
    ID3D11ComputeShader* emitShader = t.emitShader;
