...
// 剛体ソルバへ流し込む材料特性のデータ化 (消費 = Collider.physMaterial の解決、M59a2)。
// 熱・破壊系のキー (M61/M62) は将来この構造体の末尾へ追加できる
...
struct PhysMat {
    ...
    float staticFriction = 0.5f;    // μs (静止摩擦の上限、M59f2)
    ...
    // ★単位は面積あたりではなく N — **ソルバは接触面積を持っていない** (マニフォールドは
    //   最大 4 点で面積が無い) ので、「このペアが支えられる引っ張り力」を
    //   直接オーサリングする。結合則は **min** (弱いほうが勝つ = 反発と揃える)。
