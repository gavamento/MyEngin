...
//   「開」になる。音はそこを越えてよいが、敵 (CharacterController) は越えられない。上の層を
//   通る辺を許すと、流れ場が障害物を越える道を張り、SampleDirection が返す水平成分だけを
//   受け取った敵が書架の側面を押し続ける (三校 stage2 で実測: moveInput 3m/s・速度 0 のまま)。
