...
//
// ★敵同士の譲り合い: 前方 1.5m 以内に entity.index の小さい敵が居て進路の横ずれが 0.8m 未満、
//   かつ相手が同じ向きへ進んでいなければ、大きいほうが相手の居ない側へ横に退く (横が閉じて
//   いれば後ずさり)。敵にソリッド Collider を併用したとき正面衝突で永久に止まるのを防ぐ。
//   履歴を持たない (位置と相手の moveInput の純関数)。定数は AgentSystem.cpp の kYield*。
//
