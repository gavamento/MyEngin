...
// ライト選別 (M54b)。「どのライトを GPU へ送るか / どれが影を投げるか」を決める純関数。
// RenderSystem のライト収集は候補を集めるだけで、選別はここが行う。
//
// なぜ RenderSystem の外の純関数なのか:
//   ・シャドウアトラス (M54c) は「影を投げるライトの列」が frame 間で安定して
//     いることを前提に矩形を割り当てる。順序が揺れると影がポップする。
//     安定性を保証する場所は 1 箇所でないと守れない。
//   ・ECS も D3D も触らないので、入力を作って出力を検査できる (selftest の対象)。
//
// カリングもソートも描画専用でワールドハッシュには一切関与しない (M16 と同じ立場)。

// 影を投げられる局所ライト (点/スポット) の上限。
// アトラス 4096^2 (ShadowAtlas の既定) を 1024^2 タイルに割ると 16 枚 = 点光源 (6 面) 2 本 +
// スポット (1 面) 数本、で埋まる
...
// 登録順の先着で切ると、点光源が 16 本並んだシーンで太陽が落ちうる。
//
// frustum == nullptr でカリングを飛ばす (カメラ不在時。視錐台が単位行列由来のゴミになる)。
// 平行光は無限遠なのでカリング対象外。
//
// ★候補が 1 件も無いときだけ既定の平行光を 1 本補う (ライトの無いシーンでも見えるように)。「候補はあったが全部カリングで落ちた」ときに補ってはいけない —
