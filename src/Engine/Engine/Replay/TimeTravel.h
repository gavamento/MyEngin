...
//   使うため (Editor は Engine より上位なので、src\Editor\ 配下に置くと
...
//
// ---- 分岐 (M72a) ----
// 「シークで戻った後に走った tick」は記録済みの未来を捨てずに **分岐 (Branch)** として残す。
// レーンはツリーを成す:
//
...
//   編集後 = ライブ側)。区別せず編集前のスナップショットを残すと、後からその tick へ戻ると
//   **編集が黙って消え**、その先へ戻ると HASH MISMATCH になる。
//   Fork は必ず編集後の状態を **pinned** で撮り直す。
...
struct TimeTravelEntry {
    ...
    // 「途中でレーン数が変わったリング」という状態を作らないため (112B × 4 = 448B/tick)
...
public:
    ...
    //   クラッシュリング / ロールバックも同じ tick で同じものを撮るので、消費者ごとに
    //   撮ると 1 tick に 2〜3 回走る (実測 約 0.2ms/回)。撮る点は HashOf で公開してあり、呼び出し側が
